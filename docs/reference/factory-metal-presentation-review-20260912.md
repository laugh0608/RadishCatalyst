...
- 历史原配置短测超 16.7 毫秒预算，60 FPS 上限未改善；9 月 25 日第二版离线分析与首轮窗口配对已完成，本轮快样本未复现持续慢绘制。标记开销和波动根因未定，没有依据修改驱动、同步、缓冲数量、画质或生产规则；后续正式版本原配置短测已通过，四档长测首轮因失焦中断；经明确通知重开后四档各 450 秒完整通过，详见 W39 周志。
...
该方案随后分两步获授权：先取得源码与 SCons，再补齐预检发现的三项依赖。构建与窗口对照现已完成，实际结果及剩余证据缺口见下文；当时未完成的四档长测与完整 Shell Godot 套件已于 9 月 25 日通过，Windows 和亲测仍待验。
...
## 2026-09-25：第二版离线分析与首轮窗口分段完成

新增 `postwait-v2/analyze_postwait.py`、34 项离线测试及命令行检查，普通 / `-O` 模式各通过；原二进制与 8 份插桩源码 hash 一致。分段先验证区间和生命周期，再按同帧端点切分互斥跨度及未覆盖余量；超时返回非零、错误输入不生成成功报告、已有结果不覆盖。离线检查记录为 `offline-20260925-133737/`。

实测初次分析误把编号 0 当作空 drawable，第二次误要求捕获末尾全部 fence 已销毁，两份失败日志保留。核对源码后修正：非零 texture 的 drawable 编号可以为 0；`finalize_display()` 在删除 RenderingServer 后 flush，但 DisplayServer 析构及其 RenderingDevice / fence 释放在后。因此报告 3 个末尾存活身份，不虚构 free；捕获内缺失销毁导致的地址复用仍拒绝。相应反例已补入测试，未改引擎或重新开窗。

`on-20260925-133319` 经实际准备页点击完成 15 秒预热 / 45 秒生产，359,359 条事件零丢弃、写入无错；16,273 个命令创建身份和 15,371 次指针复用已关联，8,590 次等待无超时。完整状态、设置与原图核对见 [W39 周志](../devlogs/2026-W39.md)。实际未出现的 fence 地址复用、初始零等待及 frame 0 回调仍只具备离线样本证据。

- 保守时钟范围包含 5,390 个生产绘制帧，锚点不确定度 2.083 微秒。应用帧 p95 `8.600ms`；绘制跨度 p95 / 最大 `8.388 / 9.570ms`；最后一次提交后跨度 p95 / 最大 `0.011 / 6.804ms`。绘制帧与应用帧分开记录，未用分位数相减解释余量。
- 唯一超过预算的原始应用帧为 `102.525ms`，同一时间区间内自动保存 `98.630ms`、绘制 `1.743ms`；保存停顿保留于统计，不归因于 fence / GPU。该保存也不解释历史持续慢绘制。
- 初始完整快照与历史官方对照相同，最终状态仅正常时间余量不同。两张原图已实际审阅，有 8 个像素不同、最大通道差 6；无可见场景 / HUD 改变，但不宣称字节一致或推断差异原因。

本轮第二版捕获范围内的分段与身份验证完成，未复现持续慢绘制；按原快样本停止条件结束窗口诊断，不追加整体开关、GPU 采样、调度补丁或四档长测。标记开销、历史性能波动、设备析构及 GPU 完成因果仍不在本轮通过结论内；下次接续以 Daily Start 为准。

同日接续：正式版本 `official-20260925-135157` 45 秒原配置短测帧 p95 `8.680ms`，8 项通过，原图与本日诊断图字节一致。随后四档长测首档约 220.7 秒因 `focus_lost` 停止，完成档位为 0，未自动重开；部分原始帧 p95 不计验收。完整 Shell Godot 回归在首次沙箱字体错误后，经授权主机重跑 74 个入口（含导入）通过。具体证据与剩余前台时段要求见 [W39 周志](../devlogs/2026-W39.md)。
...
经萝卜SAMA明确通知，正式 Godot 原配置 `performance-1790317200-22165` 完成四档各 450 秒，29 项检查通过。帧 p95 `8.726 / 8.562 / 8.808 / 9.696ms`，60 次采样内自动保存均成功，原图和完整原始采样已核对；详见 [W39 周志](../devlogs/2026-W39.md)。本机持续性能缺口关闭，历史慢绘制和插桩开销仍未定因，不据本次达标修改渲染策略或追加同类诊断。
