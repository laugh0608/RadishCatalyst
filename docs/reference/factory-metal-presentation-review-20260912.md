...
下一步使用 `postwait-v2/run-postwait.py on` 做一轮有界分段验证，先检查事件零丢弃、成对区间、fence / command 生命周期与画面 / 状态一致性。只在实际捕获慢帧时比较同帧各段及未覆盖余量；不能将嵌套跨度相加、分位数相减或快帧结果冒称慢帧根因。若仅捕获快帧，明确记为未复现，不继续以重复开关追求稳定结果；待分段证据指出具体等待后再决定是否需要同轮 GPU 关联。
