# Design Documents

本目录用于后续收纳具体玩法与内容设计文档，包括核心循环、资源链、地图区域、角色成长、装备、战斗、交互原型、任务框架和新手引导设计。

建议使用英文文件名，例如：

- `core-gameplay-loop.md`
- `mvp-feature-list.md`
- `resources-and-process-chain.md`
- `character-progression-and-equipment.md`
- `maps-and-regions.md`
- `combat-and-interaction-prototype.md`
- `narrative-and-quest-framework.md`
- `onboarding-and-first-hour.md`

## Current Documents

- [Core Gameplay Loop](core-gameplay-loop.md)
- [Character Progression And Equipment](character-progression-and-equipment.md)
- [Combat And Interaction Prototype](combat-and-interaction-prototype.md)
- [Maps And Regions](maps-and-regions.md)
- [Narrative And Quest Framework](narrative-and-quest-framework.md)
- [Onboarding And First Hour](onboarding-and-first-hour.md)
- [Resources And Process Chain](resources-and-process-chain.md)
