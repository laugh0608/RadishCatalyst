...
- 后续实机截图确认：晶体局部问题已转出，但新档首屏仍被大区域边界、旧网格和黑色空场主导，前哨核心像浮在调试图上的 UI 圆盘。

## 2026-06-25 第十三包：基地首屏游戏化重建与调试图剥离

- 恢复前首屏不再渲染旧 `OpeningSceneLayer`、`Region*`、`RegionBoundary*`、跨屏路线和路线展示层，改为由基地视觉层直接绘制局部机库地面、地板分块、断供痕迹、管线、阴影和边缘暗化。
- 前哨核心补机器底座、夹具、底部机座和低功率信号，让核心看起来是场景设备，不是地图 UI 圆盘。
- 首小时和视觉 pass 检查新增“旧规划层不可见、恢复后路线能力恢复”的断言，防止后续又把 debug 图层带回新档首屏。
- 未新增区域、任务、资源、配方、存档字段、完整地图系统或试玩验收。
- 后续先实机复核新档首屏是否真正像游戏画面；若仍失败，继续处理角色 / 设备 / 地面主体，不推进基地入料段。

## 2026-06-25 第十四包：基地首屏专用呈现层

- 新增 `DemoBaseStartupPresentationLayer`，恢复前用专用低保真基地场景底稿覆盖旧地图：暗场背景、机库地面、分块材质、局部阴影、核心机座、线缆和设备剪影成为首屏主读法。
- 该层只在前哨核心恢复前显示；恢复后自动退场，把后续路线、区域和设备状态交还给既有视觉层。
- 首小时和视觉 pass 检查补充“专用呈现层存在、形状证据完整、位于玩家之下、修复后隐藏”的断言；静态接线检查同步脚本和场景引用。
- 未新增区域、任务、资源、配方、存档字段、完整地图系统、素材包接入或试玩验收。
- 下一步仍先实机复核新档首屏观感；若成立，再把同一低保真机器 / 材质语言推进到基地入料、反应器和整备交接段。

## 2026-06-25 第十五包：首屏主体资产与交互锚点重画

- 首屏专用呈现层提高到旧交互 marker 之上、玩家和目标引导之下，直接接管恢复前“前哨核心长什么样”，避免旧 UI 方块浮在设备上。
- 前哨核心从矩形面板重画为非矩形机壳、底座、夹具、灯点和修复端口；玩家脚下改为服务工位，地面补螺栓、划痕、污迹和碎片，减少大暗平面感。
- 启动核心目标引导不再画跨屏路线和大焦点框，改为小型修复端口与短连接线；底部 HUD 继续承担 `按 E` 文本。
- 已补视觉 pass、首小时检查和静态接线；未新增区域、任务、资源、配方、存档字段、完整地图系统、素材包接入或试玩验收。
- 下一步仍先实机复核新档首屏；若核心、玩家服务位和修复端口能成为主读法，再推进基地入料 / 反应器 / 整备交接段。

## 2026-06-25 第十六包：首屏空间填充与玩家修复动作

- 首屏专用层补低亮周边工业资产、维修箱、半埋设备、断裂管线、服务沟和深度阴影，减少右侧 / 下方大面积空黑场，让前哨读成一个受损现场。
- 前哨核心缩小并压低发光强度，保留核心主读法但不再压住玩家和修复端口。
- 玩家到修复端口新增工具线、接线点和火花反馈，强化“玩家正在修复核心”的动作关系。
- 已补视觉 pass、首小时检查和静态接线；未新增区域、任务、资源、配方、存档字段、完整地图系统、素材包接入或试玩验收。
- 下一步仍先实机复核新档首屏；若空间层次和修复动作成立，再推进基地入料 / 反应器 / 整备交接段。

## 2026-06-25 第十七包：首屏角色化与核心设备实物化

- 玩家自身外圈读法降权，补更明确的身体体块；首屏专用层在服务工位下补低保真维修姿态、头盔、背包、工具臂和脚位，让角色不再只像一个交互光标。
- 前哨核心补破损外壳、裂纹、通风缝、暴露维修端口和局部接线反馈，主读法从抽象圆环转成受损工业设备。
- 核心与玩家之间补高优先级地面材质、服务沟、螺栓、油污和近距离火花，强化“人在修设备”的动作关系。
- 已补视觉 pass、首小时检查和静态接线；未新增区域、任务、资源、配方、存档字段、完整地图系统、素材包接入或试玩验收。
- 下一步仍先实机复核新档首屏；若角色、损坏核心和修复动作成为主读法，再推进基地入料 / 反应器 / 整备交接段。
...
2026-06-30 复核后，首屏方向进一步收束到 [Demo Base First Screen Raster Art Pack V1](demo-base-first-screen-raster-art-pack-v1.md)，主读法必须来自 raster 贴图 / sprite；2026-07-01 对比截图确认第一版仍明显偏低保真贴片，后续转入 [Demo Base First Screen Asset Quality Pass V1](demo-base-first-screen-asset-quality-pass-v1.md)，重点提升地表、平台、设备体积、统一视角和接地阴影。

本专题继续作为 UI / Art 总线，负责 HUD、低保真美术和后续基地段承接；首屏具体实现边界以资产化子专题为准。

## 2026-06-27 核心写入交接段

- 污染边界短挑战完成后，局部作业面补短挑战结果盘、过滤输出、核心稳压材料清单和核心站入口端口，让玩家看到污染承压和短战斗产生了可带走结果。
- 核心稳定站补入口清单、缓冲包插槽、写入能量、恢复波和结尾异常探针，承接基地 / 整备收益进入写入、写入后阶段恢复和后续异常悬念。
- HUD 核心循环与 `现场记录` 补短句，把污染沉积、药剂、污染浆液、核心稳压缓冲包和核心写入关系压成默认画面能读到的接力。
- 实现继续复用既有区域、任务、资源、配方、敌人、设备和存档字段；检查只覆盖桥接层接线和运行时 shape，不进入试玩验收。

## 2026-06-27 Demo 结尾钩子可见收束

- 核心写入完成后，前哨基地画面新增稳定窗口、核心归档面板、复测状态、整备回路和异常探针，让回基地整理结果不只停留在 HUD 文案。
...
## 2026-06-28 实机审阅后可读性包

- 根据 2 / 3 / 4 / 6 号实机截图，当前默认路径方向成立，但晶体区过密、交互标签压在线稿上、污染短挑战右侧空场仍影响第一眼读法。
- 本包给当前聚焦交互标签补深色半透明底板和细边框，晶体层降低生态背景与矿面线权重并补前景采集锚点，污染共享资产语言补压力雾、腐蚀裂边和短挑战脉冲。
- 实现继续复用既有视觉层、交互对象、任务、资源、配方和存档字段；检查只覆盖标签底板、晶体降噪锚点和污染压力场景证据，不进入试玩验收。

## 2026-06-28 工作面大框退场包

- 根据新 2 / 3 / 4 / 6 号实机截图，核心区动态 frame 仍在晶体采集和污染边界画面里形成蓝 / 黄四边大框，压过了当前对象和地貌主体。
- 本包保留 `DemoCoreSceneSpaceLayer` 的 16 段 frame 数据和检查覆盖，但在玩家进入默认路径工作面时自动把 frame 退到近不可见层级；晶体补当前采集工作面底，污染补更靠近玩家路径的密集压力工作面。
- 实现继续复用既有区域、任务、资源、配方、敌人、设备和存档字段；检查覆盖 frame 仍存在、工作面 alpha 退场、晶体采集面和污染压力主体证据，不进入试玩验收。

## 2026-06-28 晶体采集工作面主体化包

- 根据新 #1 / #2 实机截图，晶体线稿、路线、矿面、设备和标签仍挤在同一层，当前矿脉和采集器输出托盘不像可操作工作面主体。
- 本包继续用同一程序绘制层追加当前矿脉、输出托盘、机台甲板、切噪面和焦点方块降权；自动检查通过，但新 #2 / #3 实机仍主要读成线框矿区、标签和几何叠层。
- 按 `Development Decision Gates` 判定为同类方案连续失败，晶体段停止继续追加线框 / 透明度 / 标签调参；后续转入 [Demo Crystal Workface Assetized Scene V1](demo-crystal-workface-assetized-scene-v1.md)。
