...
当前活跃专题：[Slice Recipe Processing V1](slice-recipe-processing-v1.md)——**2026-07-19 实机复核后方向修订为工厂自动化 arc 总览**：抽象实现（包 1+2，全局计数 + 反应器按 E 激活）判定层级过抽象，重写为"首条化工链的空间工厂自动化"（L0 资源模型改造 → L1 手持合成 → L2 电力 → L3 建造放置泛化 → L4 传送带物流 → L5 反应器接入收束），抽象实现保留为过渡参照。待萝卜SAMA确认 L0 起步与美术授权后另建可执行细专题实施。敌人等新世界层素材生成的视角前置已解除，生成会话由萝卜SAMA择时启动。
