...
状态：已暂停；`R1` 最终参考图复核未通过，`R2-A` 两轮生成也未产出可接入场景板。项目转入目的、结构与个人 + AI 开发方式复盘，本专题不自动续接。
...
- 设备风格锚点定稿存 `assets/reference/style-anchor.png`；宽幅场景的构图、透视、尺度与密度同时以 `assets/concept-art/2026-06-25-demo-first-screen-wide-reference.png` 为参照。
...
2026-07-12 最终复核：萝卜SAMA 以正式新档实机截图再次对照宽幅参考图，撤回上述通过结论。四要件只能证明局部没有黑底、悬浮和均匀撒点，不能证明整景达到目标；独立切图的透视 / 比例不统一、正交平台拼块、受流程显隐限制的稀疏首屏和大面积原型 HUD 仍使画面读成贴图拼盘。同一介质两轮已失败，`R1` 到此停止，不再缩平台、挪 sprite、补 tile 或调阴影。

- 不做：不新增玩法 / 区域 / 资源 / 存档字段；不新增程序绘制视觉层或色块拼装；不新建检查脚本文件。
- 原出口（最终未满足）：基地首屏满足下方四要件且参考图对比成立；同一步最多重试一次，仍不成立升级为素材 / 介质决策。

### R2 统一宽幅场景地基包

玩家目标：新档第一眼应接近宽幅参考图的完整工业前哨，而不是先识别出地砖、平台块和若干独立设备；修复核心后仍能保留同一整景，只改变状态主体与可交互前景。

- 介质：以一张真实宽幅场景板 `Sprite2D` 承载浅暖地形、不规则平台边界、道路 / 高差、静态管线、围栏、工作灯、背景设施和远景环境密度；其上叠加独立的玩家、状态核心、关键交互设备、碰撞 / 锚点和反馈层。
- 烘焙边界：场景板不得包含玩家、HUD、文字、受损 / 修复核心、会显隐或变状态的交互设备；应为核心和 3 到 4 台关键设备留下清楚插槽，并保留现有首分钟出站路径。静态背景设施可承担工业密度，但不可伪装成可交互主体。
- 统一性：场景板与交互资产都使用同一 3/4 俯视透视、左上光向、浅暖地表和暗青灰金属体系；相机尺度、角色 / 建筑比例及 HUD 占比按宽幅参考图重新校准。
- 实现边界：继续使用真实栅格资产与 `Sprite2D`，不新增 `_draw()`、`Polygon2D`、`ColorRect`、脚本生成贴图或程序绘制视觉层。`R1` 代码暂作过渡，待 `R2` 正式入口成立后再随 `P3` 清理。

串行开发包：

1. `R2-A` 场景板生成：独立图像会话只生成并落盘最多 3 张候选，不承担审阅、处理、代码接入或 Godot 验证。
2. `R2-B` 场景板审阅与接入：独立执行会话缩放检查、确认交互插槽 / 路径 / HUD 安全区，定稿后接入正式地图；不在此会话生成图片。
3. `R2-C` 交互资产生成：另开图像会话，生成同机同视角的核心状态对与同透视关键设备；玩家若现有形象无法统一，再作为单独素材类别处理。
4. `R2-D` 正式入口整合：叠加交互层、碰撞与反馈，校准相机和 HUD；从 `Boot -> GameRoot -> VerticalSliceMap` 取得新档 / 修复后截图并复核。

出口：两态截图在统一透视、尺度层级、工业密度、地形 / 平台融合和主场景占比上实质接近宽幅参考图，同时核心、玩家、能量灯和交互提示清楚可读。只满足原四要件不再算通过；同一实现方式第二轮仍失败即停手升级。

本包不做：`P2` / `P3`、玩法 / 区域 / 任务 / 存档扩展、素材包横向扩充、工程整改 `H1` 到 `H4`。

2026-07-12 最终结果：两轮各 3 张候选均无可定稿项。第二轮已能把晶体工作面移到右侧，并分别改善路径或插槽，但没有候选同时满足连续路径、`1 + 3~4` 交互插槽、静态 / 交互设施语义与 HUD 安全区。根因主要是生成模型不能稳定控制多项精确空间约束，次要是单张生成式场景板不适合作为运行时玩法装配底板；不是继续细化提示词可以解决的问题。

决策：触发两轮失败闸门，停止第三轮生成，不进入 `R2-B` / `R2-C` / `R2-D`。候选保留在 `assets/art-intake/2026-07-12-batch01/` 与 `batch02/` 作为失败证据；后续路线由 [Project Purpose And Solo AI Development Review](../planning/project-purpose-and-solo-ai-development-review.md) 重新决定。
...
`R1` 的四要件仍是必要条件，但已证明不是充分条件。`R2` 还必须满足：统一 3/4 俯视透视与尺度、地形 / 平台 / 静态工业结构同源成景、首屏工业密度接近宽幅参考图、HUD 不压缩主场景。缺任一条即判不成立，真实资产平铺或局部检查全绿都不算通过。
