...
3. 全局光照 + 色调：复用资产烘焙的统一左上光，以 `CanvasModulate` 建立整体 color grade，并用区域地面保留暖基地 / 冷晶体与污染带差异；无 normal map 时不强接只会均匀增亮的 `DirectionalLight2D`。
...
2026-07-12 进度：第 3 步已通过正式新档 / 修复核心后截图验收。首轮场景级暖色调导致全屏偏黄褐，唯一一次根因修正改为压红、保蓝的中性日光基线；活跃首屏与晶体路径 sprite 归一 RGB、保留状态 alpha，基地浅暖地面、暗青金属与晶体冷色差异成立。下一步进入第 4 步接地阴影，不继续调色。

2026-07-12 进度：第 4 步首轮通过正式新档 / 修复核心后截图验收。新增可复用 `GroundShadow` 原生 `Sprite2D` 组件，直接引用目标贴图透明轮廓，按物件高度贴合脚底并统一向右下压缩投影；首屏核心、玩家、可见基地设备和修复后晶体路径关键物件同步显隐。阴影未形成大片黑污，未遮挡平台纹理、能量灯、交互环或 HUD 提示；玩家旧 `_draw()` 圆形假阴影已移除。下一步进入第 5 步构图收拢，不继续调阴影。

2026-07-12 进度：第 5 步第二轮通过正式新档 / 修复核心后截图验收。运行时平台与晶体地面改为非对称轮廓，首屏道具收成核心主体、右侧支撑物和下方前景设备簇；修复后主晶体保持既有玩法锚点并提高视觉权重，卫星簇、工作灯和残骸沿右上 / 右下形成层次。首轮因平台下半部空金属场过大未通过，唯一根因修正只收紧平台轮廓；第二轮确认基地首屏同时满足场景成立四要件，`R1` 不再继续同类调参。
