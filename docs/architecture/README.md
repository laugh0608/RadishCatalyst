# Architecture Documents

影响工程边界、数据模型、联机策略和长期演进路线的架构文档。

- [Factory Discovery Save V2](factory-discovery-save-v2.md)（D2-A 新世界电力 / 统计联合字段，旧世界不迁移）
- [Factory World State And Save V1](factory-world-state-and-save-v1.md)（已接入的工厂状态与独立存档合同，计时和验收缺口单列）
- [Production Client Technology Assessment V1](production-client-technology-assessment-v1.md)（路线评估依据；Godot 3D 首包基础已实现）
- [Runtime Systems Overview](runtime-systems-overview.md)
- [Slice Runtime Systems](slice-runtime-systems.md)
- [Multiplayer and Save Architecture](multiplayer-and-save-architecture.md)
- [Code Style And Language Practices](code-style-and-language-practices.md)
- [Platform And Compatibility](platform-and-compatibility.md)
- [Save Data Model](save-data-model.md)
- [Static Data Schema](static-data-schema.md)
- [Godot Project Structure](godot-project-structure.md)
