...
> 本文主体描述冻结旧纵切的 `SaveService`、三槽位和 `WorldState / CharacterState` 校验。当前正式 `Boot → SliceSaveCatalog → StartupMenu → SliceWorld` 使用物理隔离的多世界目录、单世界 `SliceSaveService`、schema 10 与三份轮转备份，见 [Slice Runtime Systems](slice-runtime-systems.md#切片存档)。
...
- 2026-06-16 之后，Demo 主路径还通过 `demo_save_state_contract_check.gd` 复验关键切面：世界区域、角色状态、库存、建筑加工态、任务链、地图对象、敌人、出发整备台状态和核心资源链状态都必须能保存、读取并继续被 HUD / 设备 / 结果反馈读取。
