...
- 当前活跃专题：[首次外勤战斗与关键样本回收](../features/slice-first-field-combat-and-sample-recovery-v1.md)；D 入库与包 1 自动 / 正式入口证据已完成，等待人工复核后进入包 2。
...
- 2026-07-27 首次外勤战斗 D / 包 1 自动证据完成：四张审阅素材入库，2 催化剂显式充能、鼠标独立瞄准 / 左键续攻、`Space` 闪避、前台输入互斥和紧凑 HUD 接入；专项 52 项、完整 Godot 回归、真实 `Boot → 载入 L5 产线副本` 30 项与三态视觉复核通过，隔离主档和三份备份保留。

当前阶段：

```text
核心充能后的首次外勤战斗与关键样本回收——包 1 等待人工闸门
```
...
3. 包 1 已完成首次充能、玩家工具、输入互斥和 HUD 骨架；人工确认前不进入包 2 的敌人、撤离和样本回收。
...
- A2 V3 敌人、B2 V2 攻击 / 命中和 C2 V2 样本已复制到正式资源目录；包 1 只运行攻击 sheet。
- 包 1 专项 52 项、完整 Godot 回归、真实 `Boot → 载入 L5 产线副本` 30 项和三态视觉复核已通过；主档与三份备份保留在 `/private/tmp/radishcatalyst-combat-package1-review/`。
- 萝卜SAMA人工确认首次充能、鼠标朝向、攻击 / 闪避和 HUD 无阻碍后，才进入包 2 单敌人、受击撤离与样本回收。
