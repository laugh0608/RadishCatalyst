...
5. [设备—传送带接驳视觉 V1](../features/slice-device-conveyor-docking-visual-v1.md) 是下一可执行专题；2026-08-12 只做 P0 坐标、遮挡和排序审计，不把现行 `z_index = 1` 失败基线继续调参。
...
- 核心 / 反应器未连接短口底图是当前基线；连接态完整终端带上层覆盖已被人工否决。接驳专题确认局部覆盖介质前，不修改逻辑端口格、`connection_distance`、足印、碰撞或 schema 8。
