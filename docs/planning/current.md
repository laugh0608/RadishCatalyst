...
- 活跃专题：[配方加工 arc（空间工厂自动化）](../features/slice-recipe-processing-v1.md)——**L0 资源模型改造**（背包空间物品、起始只有受损核心）已收口（2026-07-19，萝卜SAMA实机确认）；**L1 手持合成面板**（B 开面板，晶体→机械零件→修核心 / 造采集器）已实现、待实机收口；下一层 **L2 核心功能化（电力 + 中央仓库，需美术授权）** 或 L3 由萝卜SAMA择序。各层状态 / 依赖见 arc 总览层表。抽象实现（旧包 1+2）已回退。
...
```text
切片实现：配方加工 arc（空间工厂自动化）——L0 已收口，L1 手持合成已实现待收口，下一层 L2
```

## 当前主线

1. 配方加工 arc（空间工厂自动化）分层：L0 资源模型改造 → L1 手持合成 → L2 核心功能化（电力 + 中央仓库）→ L3 建造放置泛化 → L4 传送带物流 → L5 反应器接入收束。L0 已收口、L1 已实现待实机收口。
2. 下一层 L2（核心功能化：电力 + 中央仓库）需美术授权（电线杆·中继）或 L3，由萝卜SAMA择序；玩家可见目标以正式入口实机为主证据。逐层开工前回 arc 总览层表核对状态 / 依赖，细专题只做当前层。
...
- 逐层收口按各层细专题验收；L0（背包空间物品 + 起始只有受损核心）已收口、L1（手持合成面板）已实现待实机收口。
