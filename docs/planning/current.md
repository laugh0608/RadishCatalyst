...
- 2026-06-16：「角色成长与战斗第一版」「功能 / 过渡路线支撑第一版」「非核心区域场景识别第一版」与「资源链状态第一版」达到阶段退出条件；非核心 8 区已有路线职责、场景身份、对象读法，核心资源链已有状态读法和专项检查。

当前阶段：

```text
首版 Demo 体验主干建设：资源链状态第一版
```

当前推进口径从区域路线和场景身份，切到资源链状态：玩家必须能判断晶体矿物、污染沉积物、污染浆液、基础零件、补给和核心稳压缓冲包处在哪条加工 / 整备链路。

## 当前主线

当前活跃专题是 [Demo Resource Chain State V1](../features/demo-resource-chain-state-v1.md)，2026-06-16 第一包已落地。下一轮开发前先对照 [Demo Definition V1](../features/demo-definition-v1.md) 判断剩余规格缺口，再建立新的非重复执行细专题；不继续加厚已完成的路线支撑、非核心场景身份或资源链状态同一读法点。
...
UI 和场景表现已完成第一轮原型呈现支撑。当前 UI 改动只服务当前活跃专题：核心资源链状态要自然出现在 HUD 基地摘要、设备面板和加工结果反馈里。
...
- 当前活跃专题内的资源链状态、设备面板、加工结果反馈、状态序列化和专项检查。
...
- [Demo Resource Chain State V1](../features/demo-resource-chain-state-v1.md) 覆盖 6 到 8 个核心资源 / 产物的链路状态和下一步去向。
- HUD、设备面板和加工结果日志至少各有一处能读出资源链状态，并有状态序列化专项检查覆盖。
- 未引入新资源、配方、设备、新任务链、完整背包、自动化物流、行动台或高压窗口扩展。
