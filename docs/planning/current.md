...
当前不切换到试玩准备或修 bug 阶段；优先把 `基地出发 -> 手持采样 -> 采集设备自动开采 -> 回基地收料 -> 基础反应器 -> 仓储输出 -> 出发整备收益` 做成第一段可跑的工业路径，并按手采、采集器建造 / 出料、回基地收料、反应器进料 / 加工、产物入库和整备交接显示当前操作段。前哨核心恢复前，首屏交互和画面都应固定到前哨核心，普通设备、路线 cue 和敌人不得抢焦点；状态反馈必须少色、少层、单主信号。
