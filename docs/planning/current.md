...
- 当前活跃专题：[首次外勤战斗与关键样本回收](../features/slice-first-field-combat-and-sample-recovery-v1.md)；包 2 人工反馈修正与证据完成，等待复核确认。
...
- 2026-07-27 首次外勤战斗包 1 人工通过；包 2 接入唯一敌人、敌我伤害、闪避免伤、撤离重试、唯一掉落和任务样本拾取。首次人工复核发现中文缺字与敌人阴影偏移；已内置中文字体并按动画单帧高度修正阴影，专项 53 项、完整 Godot 回归、真实 `Boot → L5 副本` 56 项和修正复核 10 项通过，等待人工确认。

当前阶段：

```text
核心充能后的首次外勤战斗与关键样本回收——包 2 修正复核闸门
```
...
3. 包 2 已完成实现、首次人工反馈修正与自动 / 正式入口证据；复核确认前不进入基地交付与 schema 7 包 3。
...
- 包 2 专项 53 项、完整 Godot 回归和真实 `Boot → 载入 L5 产线副本` 56 项通过；首次人工反馈的中文缺字与阴影偏移已修正，正式入口修正复核 10 项及两态联系表通过。
- 复核世界保留在 `/private/tmp/radishcatalyst-combat-package2-review/`；原证据与修正证据分别位于 `assets/art-intake/2026-07-27-combat-package2-preview/` 和 `assets/art-intake/2026-07-27-combat-package2-review-fix/`。
- 等待萝卜SAMA复核确认；包 2 不做重启持久化，`dropped / carried` 重启矩阵随包 3 schema 7 验证。
