...
```text
首版 Demo 表现层重建第一版：画面地基重建（地形 / 平台 / 光影 / 接地阴影 / 构图）
```
...
4. `R1` 地形+光影地基包（当前）：按"地基不成立就不摆物件"的铁律，依次建①连续地形铺满相机视野②基地平台落座面③统一光向与色调（暖基地 / 冷污染带）④每个物件接地阴影⑤构图收拢（焦点 + 疏密 + 前中后景）；前四步是"像不像游戏"的主体。同一步最多重试一次，仍不成立升级为素材 / 介质决策，不做同类调参循环。
5. `P2` 三区扩展包（暂停复判）：待 `R1` 四要件成立后，再判断三区连续路径是否成立。
6. `P3` 旧层清除收口包（待 `R1` 与 `P2` 复判成立后开工）：删除旧视觉层脚本与节点，修剪失效检查。
