...
2. 包 2 无缝大地图与相机跟随：基地与晶体区合并为一张连续地图（"单基地 + 单远征区"= 同图两区域，2026-07-16 定），相机挂玩家、按地图边界钳制。
3. 包 3 同构验证与收口：两区与过渡带截图对比证据轮预览，无缝往返路径实机复测，周志收口。
...
- 专题三包全部收口：晶体区场景成立、正式入口新档基地 ↔ 晶体区无缝步行往返、两区与证据轮预览同构。
- 收口截图与验证记录写入当周周志。
