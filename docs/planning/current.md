...
- 已补窗口长帧计时、规模拆改、四侧边缘移动 / 相机 / HUD 及原图证据；系统鼠标打开二次确认与取消已验证。窗口批次仍有弹窗注入失败、原生操作等待超时及启动失焦中断，不能判整轮通过，原生重试 / 最终放弃仍待补验。
...
接续按 [Daily Start](daily-start.md) 补齐正式 Boot 的原生重试保存、最终放弃与完整返回路径，再执行首档四档前台持续性能。计时 / 暂停、规模拆改和地图边缘已有部分窗口证据；后台长测不替代原生焦点、鼠标命中或渲染性能。
