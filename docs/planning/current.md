...
- 2026-07-07 `R0` 代码基线已推进：正式 `Boot` 入口可见，首分钟显隐与旧层硬禁用已接入，修复核心后出现晶体路径视觉，晶体任务当前目标选择遵守朝向规则；尚未以新档第一分钟截图序列验收。
...
3. `R0` 首分钟体验重基线包（当前）：新档开局只读损坏核心；修复后放开晶体 / 残骸采集与晶体路径视觉；硬禁用旧色块和语义轮廓，收紧交互，等待新档第一分钟截图序列验收。
