...
- 当前活跃专题：[视觉层级与色彩分离 V1](../features/slice-visual-hierarchy-and-color-separation-v1.md)；关键设备第 3 轮 V3 的几何、比例和色调获得条件认可，滚轮式端口与四向对称接口判废。8 月 4 日进入固定正面与可见侧端口小样，世界 Gate A 与 UI Gate B 均未批准。
...
3. [视觉层级与色彩分离 V1](../features/slice-visual-hierarchy-and-color-separation-v1.md) 8 月 4 日只推进核心 / 反应器端口小样：保留 V3 的轴向、高度、浅色机身和深色底座，移除设施滚轮，使用方向箭头；设备正面固定，端口只围绕左、右、前三侧。核心表达真实仓储 `IN / OUT`，反应器表达加工 `IN / OUT`。新轮次从 `0 / 3` 开始，批准前不裁切 / 归一或重组 Gate A。
...
- 现行客户端仍以 `building_rotation` 同时驱动设施贴图、端口、供电、调整与存档；核心仓库也尚无物流网端点。明日小样只验证固定正面与可见侧接口，不宣称功能已实现；正式改动前必须另做核心物流、显式端口和存档兼容的架构级迁移，不能由美术小样静默改写。
