...
- 2026-07-08 `R0` 首轮实机截图复核暴露靠近晶体时旧层回流、晶体路径不够可读；已补运行时旧层压制、核心到晶体中段资产路径和修复前 / 修复后显隐检查，仍需重跑新档第一分钟截图序列验收。
...
3. `R0` 首分钟体验重基线包（当前）：新档开局只读损坏核心；修复后放开晶体 / 残骸采集与晶体路径视觉；硬禁用旧色块和语义轮廓，收紧交互，并在最新代码补过旧层回流压制和中段路径资产，等待新档第一分钟截图序列复验。
