...
- 2026-06-21：多轮定位截图后的视觉修正已落地，并继续完成 2 号晶体点位与 3 号污染边界治理；新截图复核后确认不再把四个定位点微调作为主目标，当前转向第一条工业路径横切，把基地出发、晶体 / 残骸、基地收料、反应器进料 / 加工、仓储输出和整备收益压到同一段玩家可跑画面里；首段状态层已从多色叠加改为背景降权、局部工作面、低饱和轮廓和单主信号。根据新档首屏截图反馈，前哨核心恢复前改为专用核心恢复视图，暂不显示完整设备层、路线 cue、侧边设备 marker 和敌人；恢复后资源段已补手持采样点、采集器工作面、出料托盘和回基地装车端口，后续再接真实建造 / 产出逻辑。
...
当前不切换到试玩准备或修 bug 阶段；优先把 `基地出发 -> 手持采样 -> 采集设备自动开采 -> 回基地收料 -> 基础反应器 -> 仓储输出 -> 出发整备收益` 做成第一段可跑的工业路径，并按手采、采集设备出料、回基地收料、反应器进料 / 加工、产物入库和整备交接显示当前操作段。前哨核心恢复前，首屏交互和画面都应固定到前哨核心，普通设备、路线 cue 和敌人不得抢焦点；状态反馈必须少色、少层、单主信号。
