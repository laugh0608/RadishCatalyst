...
- 存档运行时复验已完成第一轮拆分；任务链、`unlocked_effects` 与区域 / 配方来源追溯用例已统一收拢到任务解锁效果 suite，后续新增跨块关系继续按职责扩展独立 suite，避免重新堆回 `save_service_check.gd`。
