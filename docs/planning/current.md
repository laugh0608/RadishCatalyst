...
- 2026-08-11：P2-B 制造 / 背包与 P2-C 设备面板依次完成并获人工通过；P2-D 核心仓库、动态减半拖拽和首次充能确认完成实现与行为验证，等待整面定稿。空口短接口恢复成立，但整格终端带上层覆盖会遮住设备主体，已升级为 [设备—传送带接驳视觉 V1](../features/slice-device-conveyor-docking-visual-v1.md)，P2-E 暂停。
...
4. [游戏本体 UI 视觉定稿 V1](../features/slice-game-ui-visual-finalization-v1.md) P0、P1、P2-A、P2-B 与 P2-C 已完成并获人工通过；P2-D 核心交互已完成双栏整栏投放、动态减半、首次充能确认、自动与正式入口验证，当前只等待整面人工定稿。P2-E 系统界面、有限发电 / 负荷 / 储能和陌生玩家盲测继续暂停。
5. [设备—传送带接驳视觉 V1](../features/slice-device-conveyor-docking-visual-v1.md) 的反应器与核心代表分面已完成专属端点几何、自然排序、完整 Godot、正式 `Boot` 与逐图复核，并于 2026-08-13 获萝卜SAMA人工通过。采集器和储物箱不主动外推修改；当前执行重新回到 P2-D 整面人工定稿。
...
- 现行客户端已使用逐类 profile、schema 8、固定正面设施与统一运行时端点；采集器右侧输出、储物箱模式端口和修复态核心左 `IN` / 右 `OUT` 均已启用。核心仍是地图固定设备，专属视觉排序壳、端点与相邻终端带关系均由运行态派生，不进入 schema 8。
