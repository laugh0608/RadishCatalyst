...
- 当前活跃专题：[第一可玩切片全链串通](../features/slice-first-playable-journey-v1.md)；包 1 待执行真实新档基线与缺口审计。
...
```text
第一可玩切片全链串通——包 1 真实新档基线与缺口审计
```

## 当前主线

1. 从真实 `Boot → 新建世界` 串通“修核心 → 化工链 → 外勤 → 样本交付”，不注入最终状态。
2. 包 1 先建立时间、资源、交互、存读和理解缺口基线，不预判要新增功能。
3. 只把 `P0` / `P1` 阻断项带入包 2；内部全链通过后才进入陌生玩家盲测。
...
- 从真实 `Boot → 新建世界` 完成核心修复、自然化工链、2 催化剂充能、唯一敌人、样本拾取和基地交付，不直接注入最终状态。
- 记录阶段时间、资源收支、交互次数、自然保存点和需要猜测的位置；至少完成一次保存返回、完全重启和载入继续。
- 核心修复、产线运行、工具充能、样本携带和交付完成态均有正式入口截图证据。
- 输出 `P0` / `P1` / `P2` 缺口清单；包 1 只建立基线，不顺手扩内容。
