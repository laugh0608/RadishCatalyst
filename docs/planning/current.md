...
- 2026-07-06 `P2` 三区定点截图已成立；下一步录制第一段 60 秒实机录像，录像成立后再进入 `P3` 旧层清除收口。
...
2. `P1` 首屏重建包（已接入）：`TileMapLayer` 地面 + 设备 sprite 场景 + 玩家 / 敌人 sprite 已覆盖默认首屏，不继续同类首屏调参。
3. `P2` 三区扩展包（当前）：晶体矿脉、污染边界、核心稳定站定点截图已成立；下一步只录制 60 秒实机录像作为主进度证据。
4. `P3` 旧层清除收口包（待录像成立后开工）：删除旧视觉层脚本与节点，修剪失效检查。
