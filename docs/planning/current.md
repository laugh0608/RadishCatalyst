...
- 当前活跃专题：[L4 传送带物流](../features/slice-conveyor-logistics-v1.md)。包 1–3 的直线、转角、端点、合流、公平轮询与 schema 5 自动 / 正式入口证据已完成；等待萝卜SAMA人工确认后收口并切换 L5。arc 总览见[配方加工](../features/slice-recipe-processing-v1.md)。
...
```text
切片实现：配方加工 arc（空间工厂自动化）——L0–L3 已收口，L4 传送带物流进行中
```

## 当前主线

1. 配方加工 arc 分层：L0 资源模型 → L1 手持合成 → L2 核心直供 + 中央仓库 → L3 建造放置 + 扩展供电网 → L4 传送带物流 → L5 反应器接入。L0–L3 已收口。
2. L4 包 1–3 实现与自动 / 正式入口证据已完成；当前只做萝卜SAMA人工复核和必要修正，通过后收口 L4，再按文档先行建立 L5 可执行细专题。
3. 敌人等新世界层素材生成的视角前置已解除，生成会话由萝卜SAMA择时并行启动（为后续战斗专题备料）。

## 边界与冻结

- 配方加工 arc 的分层顺序、资源模型改造（L0）、美术授权均属架构级升级点；2026-07-25 萝卜SAMA确认 L3 收口并授权进入 L4，专题与闸门已建立。L4 首选确定性派生素材；具体图像生成仍须另行授权并按专题批次拆分与协作上限执行。
...
- 逐层收口按各层细专题验收；L0–L3 已收口。L4 出口按其细专题覆盖玩家装料、可见物品搬运、方向 / 转角 / 合流、端点入库、回压守恒与 schema 5 重启还原。
