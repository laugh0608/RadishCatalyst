...
当前不切换到试玩准备或修 bug 阶段；优先把 `基地出发 -> 手持采样 -> 采集设备自动开采 -> 回基地收料 -> 基础反应器 -> 仓储输出 -> 出发整备收益` 做成第一段可跑的工业路径，并按手采、采集器建造 / 出料、回基地收料、反应器进料 / 加工、产物入库和整备交接显示当前操作段。下一轮先用 1 / 3 / 4 号截图点核对首屏、采集器出料和基地交接；读法成立后再推进设备状态灯、资源 / 浆液流动、敌压警告和核心写入反馈。
