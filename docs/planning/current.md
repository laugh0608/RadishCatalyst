...
- 当前下一步：为配方加工 arc 建立 **L4 传送带物流**可执行细专题，先过玩家路径、物流状态 / 存档、端点与转角素材及正式入口证据的开发闸门；未建专题前不写物流代码。arc 总览见[配方加工](../features/slice-recipe-processing-v1.md)。
...
```text
切片实现：配方加工 arc（空间工厂自动化）——L0–L3 已收口，下一步建立 L4 传送带物流专题
```

## 当前主线

1. 配方加工 arc 分层：L0 资源模型 → L1 手持合成 → L2 核心直供 + 中央仓库 → L3 建造放置 + 扩展供电网 → L4 传送带物流 → L5 反应器接入。L0–L3 已收口。
2. 下一步先建立 L4 可执行细专题并做开发决策闸门，不把旧打磨观察项或同层视觉微调插到物流主线之前。
3. 敌人等新世界层素材生成的视角前置已解除，生成会话由萝卜SAMA择时并行启动（为后续战斗专题备料）。

## 边界与冻结

- 配方加工 arc 的分层顺序、资源模型改造（L0）、美术授权均属架构级升级点；2026-07-25 萝卜SAMA确认 L3 正式入口视觉通过。进入 L4 前先建立可执行专题并确认闸门；具体图像生成仍须按专题批次拆分与协作上限执行。
...
- 逐层收口按各层细专题验收；L0–L3 已收口。L4 的当前出口须在新专题中定档，至少覆盖可见物品搬运、方向 / 转角 / 合流、端点入库与存读还原。
