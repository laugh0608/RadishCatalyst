...
- 当前活跃专题：[Demo Presentation Rebuild V1](../features/demo-presentation-rebuild-v1.md)，覆盖表现层重建、真实资产管线与首分钟体验重基线。
...
1. `P0` 素材锚定包（主体完成）：保留 1 到 2 个免费素材包评估作为角色动画补位与兜底。
2. `P1` 首屏重建包（已接入但不成立）：真实资产已覆盖首屏，但设备堆叠、语义轮廓和首分钟流程仍破坏读法。
3. `R0` 首分钟体验重基线包（当前）：新档开局只读损坏核心；修复后才放开晶体 / 残骸采集；硬禁用旧色块和语义轮廓，收紧交互，放大 HUD。
4. `P2` 三区扩展包（暂停复判）：待 `R0` 通过截图序列后，再判断三区连续路径是否成立。
5. `P3` 旧层清除收口包（待 `R0` 与 `P2` 复判成立后开工）：删除旧视觉层脚本与节点，修剪失效检查。
...
- 旧视觉层和交互语义轮廓先硬禁用，不再以"降权"方式与新介质长期共存；后续 `P3` 删除文件。
...
- 本周主进度证据先改为新档第一分钟截图序列；`R0` 成立后再恢复 60 秒路径录像。
...
- 首分钟流程可调整实体显隐、交互范围、HUD 可读性和核心后续目标；不在本包实现完整科技树、装备栏或工具面板。
...
- 新档第一分钟能读成"损坏核心 -> 修复 -> 去晶体矿脉采集"，没有旧流程图色块和过早设备堆叠抢读。
