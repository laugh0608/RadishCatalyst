...
- 活跃专题：[配方加工](../features/slice-recipe-processing-v1.md)（化工链第二步、首次工业转化）——专题文档已建立，四项关键取舍已定档（催化剂 / 自动加工 / 核心充能 / 储存罐容量上限），待萝卜SAMA确认后进入实施。
...
```text
切片实现：配方加工（化工链第二步）——专题已建立、关键取舍已定档，待确认后实施
```

## 当前主线

1. 配方加工专题文档已建立、四项关键取舍已定档；待萝卜SAMA确认后按专题分包实施（包 1 反应器自动加工与催化剂储存 → 包 2 核心充能与产线闭环 → 包 3 全链路验证收口）。
2. 按专题分包实施至收口；玩家可见目标以正式入口实机为主证据。
3. 敌人等新世界层素材生成的视角前置已解除，生成会话由萝卜SAMA择时并行启动（为后续战斗专题备料）。

## 边界与冻结

- 配方加工不生成新素材（反应器 / 储存 / 整备台已在场，新资源用 HUD 文本表达）；世界层新素材生成一律按高斜角投影口径执行。
...
- 配方加工专题关键取舍经萝卜SAMA确认，进入实施。
- 专题收口标准以专题文档为准：正式入口"采集 → 自动产出 → 加工 → 核心充能"完整链路实机成立并可存读还原。
