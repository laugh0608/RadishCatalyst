...
- 按已定本地存档策略，在建筑状态取值与配方归属约束基础上继续补齐设备运行时来源、加工中进度字段和更细的跨块关系边界。
- 继续扩展第一切片关键流程复验，让任务链、存档状态和后续 UI 表现逐步覆盖更多真实游玩路径。
- 继续保持污染过滤器为最小加工点，不展开完整设备 UI、自动化、电网或管线。
- 补齐更清楚的交互反馈、正式教程提示和更正式的任务完成界面表现。
- 在地图对象、敌人和建筑运行时来源 / 字段 / 取值复验已起步的基础上，继续补齐加工中状态、设备进度和更多跨块关系边界。
...
- 可运行原型骨架已能启动、显示 HUD、移动玩家，并通过静态数据驱动采集 / 采样、工具标签校验、基础攻击、敌人反击、快捷栏补给、最小基地加工、设备缺料提示、过滤模块启用、地块清理、地基铺设、污染处理点建造 / 启用、污染失败再尝试引导、原型任务推进、区域通行门槛 / 方向提示、切片结尾入口提示、切片完成 HUD 状态、默认槽位存档保存 / 读取失败提示、最近 3 份备份轮转、坏档备份读取恢复、旧原型存档迁移、首批存档内容一致性校验、首批实例与跨块关系校验、地图对象 / 敌人 / 建筑来源复验、地图对象 / 敌人 / 建筑字段白名单校验、建筑状态取值与配方归属校验、已完成任务目标满足度校验、任务目标类型复验、切片结尾状态复验和本地存档阶段策略。
