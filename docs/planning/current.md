...
- 当前活跃专题：[Demo Map Surface Decomposition V1](../features/demo-map-surface-decomposition-v1.md)，拆分已接近硬上限的地图区域 / gate 承载面，支撑后续场景和路线推进。
- 最近完成：[Demo Interaction Prompt Surface Decomposition V1](../features/demo-interaction-prompt-surface-decomposition-v1.md)、[Demo Route Return And Base Reentry Readability V1](../features/demo-route-return-and-base-reentry-readability-v1.md)、[Demo Functional Transition Spatial Playability V1](../features/demo-functional-transition-spatial-playability-v1.md)、[Demo Functional Scene Gameplay Density V1](../features/demo-functional-scene-gameplay-density-v1.md)、[Demo Supply Pressure Pacing V1](../features/demo-supply-pressure-pacing-v1.md)
...
- 2026-06-19：功能 / 过渡场景玩法密度、可达空间、外勤返回与基地再进入读法、交互提示承载面均通过退出判断，当前切到地图承载面拆分第一版。

当前阶段：

```text
首版 Demo 体验主干建设：地图承载面拆分第一版
```

当前推进口径是地图承载面拆分第一版：不新增资源、配方、设备、区域、任务链或敌人类型，先拆出地图区域 / gate helper，降低 `vertical_slice_map.gd` 行数风险。

## 当前主线

当前活跃专题是 [Demo Map Surface Decomposition V1](../features/demo-map-surface-decomposition-v1.md)。它承接已完成的交互提示承载面拆分与运行时承载面拆分原则，覆盖 [Demo Definition V1](../features/demo-definition-v1.md) 的「区域 / 场景」「自动检查」和工程承载面缺口。
...
- 地图承载面拆分第一包：地图区域判断、gate 回退、回投坐标和对象区域归属迁出 `vertical_slice_map.gd`，保留原入口和玩家路线。
...
- [Demo Map Surface Decomposition V1](../features/demo-map-surface-decomposition-v1.md) 建立并完成地图区域 / gate 承载面拆分。
- 区域判断、gate 回退、回投坐标和对象区域归属保持一致，`vertical_slice_map.gd` 明显低于硬上限。
