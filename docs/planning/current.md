...
- 当前活跃专题：[L5 反应器接入自动化](../features/slice-reactor-automation-v1.md)。开工闸门、固定配方、双端口、schema 6 与三个实现包已定；催化剂货物素材硬闸门已通过，下一步进入包 1 的反应器实例 / 存档代码。arc 总览见[配方加工](../features/slice-recipe-processing-v1.md)。
...
2. L5 已建立可执行细专题且催化剂货物素材硬闸门已通过；现按包 1–3 接入反应器双端口、固定加工、schema 6、机器物流和完整产线人工收口。
3. 敌人等新世界层素材生成的视角前置已解除，生成会话由萝卜SAMA择时并行启动（为后续战斗专题备料）。

## 边界与冻结

- 配方加工 arc 的分层顺序、资源模型改造（L0）、美术授权均属架构级升级点；2026-07-25 萝卜SAMA确认 L4 收口并进入 L5，专题与闸门已建立。2026-07-26 催化剂货物独立批次已获授权并收口，后续不追加同类生成；反应器加工反馈仍按确定性离线派生执行。
