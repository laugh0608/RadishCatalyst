...
- 2026-07-04 风格锚点定稿：第一批 6 个 AI 反应器候选全部达可用线，`a0_reactor_v4` 定为锚点存 `assets/reference/style-anchor.png`；第二、三批主候选已审定并处理到 `client/assets/`，`P1` 首屏重建可开工。
...
1. `P0` 素材锚定包（主体完成）：保留 1 到 2 个免费素材包评估作为角色动画补位与兜底，不阻塞 `P1`。
2. `P1` 首屏重建包（下一步）：`TileMapLayer` 地面 + 设备 sprite 场景 + 玩家 / 敌人 sprite，首屏范围关闭旧视觉层。
