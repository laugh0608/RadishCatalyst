...
- Godot 客户端工程已在 `client/` 初始化，并已建立第一可玩切片的静态数据、启动链、临时地图、原型 HUD、玩家移动、采集 / 采样规则层、基础战斗命中和基础反应器多配方轮换加工雏形。
...
- 在基础反应器多配方命令已接通后，继续补齐基础过滤模块对污染边界通行、防护或任务推进的实际影响。
- 视切片推进需要，接入基础过滤模块或污染过滤器的最小启用命令，但暂不展开完整设备 UI、自动化、电网或管线。
