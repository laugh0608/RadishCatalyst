...
- 当前活跃专题：[首次外勤战斗与关键样本回收](../features/slice-first-field-combat-and-sample-recovery-v1.md)；包 2 人工复核通过，准备进入包 3。
...
3. 包 2 已通过人工闸门；下一包接入基地交付、永久生命收益与 schema 7，不扩大敌人或成长系统。
...
- schema 7 严格保存玩家生命与 `locked / hostile / dropped / carried / delivered` 遭遇状态，schema 2–6 迁移、坏候选回退和多世界隔离保持成立。
- 核心交付优先于充能 / 仓库，交付后抗蚀内衬把最大生命固定提升到 `120`；伤害、败亡、拾取、交付和撤离完成后自动保存。
- 中途战斗、掉落、携带、交付、撤离重启矩阵通过；真实 `Boot` 从充能跑到交付，交付前后 HUD / 核心反馈截图和保留存档完备，再交人工复核。
