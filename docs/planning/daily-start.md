...
9 月 13 日已获原隔离诊断方案授权，固定源码下载与 SCons 4.11.1 安装完成。构建预检缺少 AccessKit、ANGLE 和 MoltenVK；新增依赖的固定版本 / 命令已补入诊断记录，待范围授权后接续。尚未编译或开窗。
...
1. **先补齐新增构建依赖的范围授权**：源码压缩包 66.7MiB，SCons 已隔离安装；预检退出 255，明确缺少 AccessKit 0.21.2、ANGLE chromium/7219 和 MoltenVK。新增依赖下载合计上限拟为 512MiB，整个隔离目录上限仍为 20GiB。具体命令与停止条件见诊断记录；不将自动关闭缺失功能后的构建作为同配置对照。
...
- 不推进 1,000 台 / 10,000 带扩展档；新增三项构建依赖在上述授权前不安装，打包、发布与推送未授权。
