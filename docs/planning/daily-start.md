...
- 2026-06-22 复核 3 / 4 号读法后停止同层截图微调，已完成第一工业路径交互层级收束，并补设备状态灯、物料槽和短资源流第一包。
...
- 当前不切换专题；优先复核 `基地出发 -> 手持采样 -> 采集设备自动开采 -> 基地收料 -> 基础反应器 -> 仓储输出 -> 出发整备收益` 的设备状态灯、物料槽和短资源流，确认 3 / 4 号不再只读成路线图。
...
- 若状态灯和资源流动读法成立，下一包把同一套视觉语言延伸到污染链、敌压警告和核心写入反馈。
