...
更新时间：2026-09-13（开销对照中断，提交后跨度已收窄）
...
9 月 13 日已完成获授权的隔离引擎构建与五次同状态窗口对照。事件验证支持“获取 drawable 后才进行本帧图编码 / 提交”，但不能证明这是长帧主因；重复官方与标记关闭组波动明显，标记开销尚未单独量清。构建后目录占用约 8.23GiB。晚间追加关闭 / 开启两组短测均通过，第三组失焦后停止，四组对照未完成；详细数据见呈现诊断记录。
...
1. **补齐稳定开关对照**：晚间同一二进制关闭 / 开启帧 p95 为 `8.564 / 8.548ms`，两组各 8 项通过；第三组在约 30.2 秒失焦，第四组未启动。独立记录器微测及两组接近结果不代表完整开销检查通过。窗口可用时重新告知并进行完整受控对照；中断批次与所有失败保留，不自动重开。
2. **核对提交后的等待与收尾**：本次开启快样本同样先获取、后提交，说明顺序本身不足以解释卡顿。最后实际 commit 返回至 `_draw` 结束的同帧跨度 p95 在旧慢 / 新快样本分别为 `28.277 / 0.0065ms`；源码范围包括帧 fence 等待、资源整理及绘制后回调，尚未归因。稳定对照后围绕该范围补证据并与同轮 GPU 活动关联，再判断是否有可修正收益；不提前修改提交策略。
