...
- [远程武器与工业供弹 V1](../features/slice-ranged-weapon-and-industrial-ammunition-v1.md) P0 与目标稿 V3 枪械身份已人工确认；原生尺寸 V1—V3 均已否决，删臂拼接路线停止，等待换介质确认。
...
1. 先复核 [远程武器与工业供弹 V1](../features/slice-ranged-weapon-and-industrial-ammunition-v1.md) P1 路线复盘，只做一个架构决定：是否批准从删臂 + 覆盖层切换为侧向源 / 朝上 / 朝下三组完整人物持枪帧。
2. 若批准换介质，另开独立图像生成会话，把“完整人物持枪帧”视为新素材类别并从自己的 `0 / 3` 计数；生成会话只负责候选、落盘和 manifest，不接客户端或 Godot。
3. 新候选必须先同时通过人体轮廓、肩—肘—腕—双手受力链、枪口朝向、前臂高度和 `48×64px` 世界尺度，再讨论左右镜像、动画帧或运行时接入。
4. 不恢复原生尺寸 V1—V3，不继续删臂、挪锚点或局部覆盖层调参；核心 / 反应器接驳、P2-D 与 P2-E 均保持已收口，不主动外推新包。
