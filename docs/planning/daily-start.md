...
主目标是先建立“建造 / 合成 / 背包图形化 UI V1”可执行专题，不立即写代码。

1. 定义建造 / 合成 / 背包图形化的玩家结果、UI 介质、鼠标键盘路径、现有权威库存复用、截图证据和失败判据。
2. 第一实现包只替换纯文字主读法：配方卡 / 物品格、材料消耗、可制作 / 不足、已有套件选中与当前放置对象；不改配方、容量或 schema。
3. 设备操作面板统一和世界 3/4 体积感分别建立后续专题，避免同包混改 UI 组件与世界资产投影。
