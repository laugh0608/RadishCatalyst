...
2. **下一步实施包 1：UI 承载层级。**只统一 HUD、制造面板与建筑设备面板的壳层、卡片、状态、操作和结果角色；先取正式基线，再改共享视觉资源和三类现行界面。
3. **包 1 人工通过后再进入世界资产包。**设备与地表分离不和 UI 混改；视觉专题收口后才建立有限电力专题。设备面板包 2、陌生玩家盲测、新内容、发布与打包继续冻结。
