...
阶段：第一可玩切片全链串通，包 1 待执行真实新档基线与缺口审计。

- [第一可玩切片全链串通](../features/slice-first-playable-journey-v1.md) 已建立；当前不增加功能，先验证新档 20 到 40 分钟完整旅程。
...
主目标是从真实 `Boot → 新建世界` 建立完整切片基线，不注入最终状态。

1. 依次完成核心修复、自然化工链、2 催化剂充能、战斗、样本拾取和基地交付。
2. 记录各阶段时间、资源收支、交互次数、自然保存点和理解断点；中途完成一次重启续玩。
3. 五个关键节点抓图并合成联系表，只输出 `P0` / `P1` / `P2` 缺口，不在审计中扩内容。

## 防跑偏规则

- 多世界与 schema 7 已收口；新专题只按明确迁移策略扩展当前切片状态，不复活旧 `SaveService`、旧 `GameRoot` 或旧三槽存档。
- 包 1 只建立真实新档全链基线；若发现引导阻断，先记录，包 2 再按现有状态派生固定目标，不复活旧通用任务链。
...
- 内部全链通过前的朋友盲测、试玩分发准备与发布。

## 必读与选读

日常必读：

- `docs/planning/current.md`（当前阶段与包 1 退出条件）
