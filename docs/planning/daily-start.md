...
- 首版 demo 已按 12 个区域封顶；当前算基地共 11 个区域，第 12 个暂定为 `region.demo_stabilization_core / 核心稳定站`。
- UI baseline 第一包已先把主 HUD 标题、目标追踪、运行时提示和任务完成反馈切到新区域口径；配方 / 物品 / 敌人名中的旧“相位井”链暂留后续文案包。

## 明日事项

1. 启动核心稳定站客户端第一包：补 `region.demo_stabilization_core`、地图标记、本地化名和最小 HUD 指向。
2. 设计核心稳定站三段任务：进入区域、击败阶段守卫、写入核心设备并触发 demo 完成反馈。
3. 启动 4 个核心区域的第一轮尺度规划：基地平台、晶体矿脉区、污染边界区、核心稳定站。
4. 对 UI baseline 第二包做一次人工短跑，重点看关键资源和基地摘要是否能指导回基地制造。
