...
- 当前首要问题仍是纵切默认画面是否稳定像游戏；下一步优先复核基地交接专用工作面是否在正确 #2 / #3 位触发。
- 优先推进低保真游戏画面成型：反应器料斗、仓储供给、整备锁扣、局部设备地面、角色动作、材质、阴影和短反馈。
- HUD、读法、状态整理和检查只能支撑画面成型，不能作为当前主线开发包。
- 每个开发包必须回答：玩家会看到什么、做什么、理解什么。

## 明日事项（2026-06-30）

1. 先按 `Demo Base Handoff Assetized Scene V1` 复核正确的基地交接 #2 / #3 位：目标、交互对象和小地图应进入基地交接语境，而不是晶体 / 污染边界语境。
2. 若专用工作面没有触发，优先修正状态触发、复测位或可见层级，让反应器料斗、仓储供给和整备锁扣成为截图主体；不新增资源、任务、区域或第三个晶体美术包。
3. 只有基地交接截图成立后，才推进污染处理 / 核心稳压段的同类场景化连续性。
