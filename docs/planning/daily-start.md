...
阶段：第一可玩切片全链串通；包 3A 前段修正已经自动与人工通过，当前恢复包 3 完整人工全链验收。

- [第一可玩切片全链串通](../features/slice-first-playable-journey-v1.md) 已把自动补地板、物流端口和采集器显式取料合入包 3A 正式入口基线；朋友盲测仍冻结。
...
主目标是恢复包 3 完整人工基线，不直接进入朋友盲测。

1. 从项目内 `manual-save` 保留世界继续双批反应器加工、催化剂取用、核心充能、首次外勤和样本交付。
2. 在加工中与最终完成态分别返回菜单并重新载入，核对进度、`delivered / 120`、世界摘要和三份备份。
