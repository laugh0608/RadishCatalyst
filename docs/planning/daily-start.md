...
- [首次外勤战斗与关键样本回收](../features/slice-first-field-combat-and-sample-recovery-v1.md) B1 已完成 `3 / 3`；等待独立 B2，不进入实现。
...
主目标是启动独立 B2 工具挥击 / 命中反馈审阅与归一，不进入实现。

1. 清点 batch03 三稿，按目标尺寸、色板、Alpha、帧差异和 HUD 安全区硬判。
2. 优先验证 V2，但不把 B1 初排当作正式选定；审阅失败时不自行续生成。
3. 通过后归一入库并更新 manifest；不混关键样本，不接玩法代码。

## 防跑偏规则

- 多世界与 schema 6 已收口；新专题只按明确迁移策略扩展当前切片状态，不复活旧 `SaveService`、旧 `GameRoot` 或旧三槽存档。
- B1 已在 `3 / 3` 停止；B2 只审阅 / 归一战斗反馈，不续生成、不接入代码；旧实现只借规则语义。
