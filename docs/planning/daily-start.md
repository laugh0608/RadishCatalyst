...
- 活跃细专题 [L3 建造放置 + 扩展供电网](../features/slice-building-placement-and-power-grid-v1.md)：六类建筑的配方 / 套件、网格放置、旋转、碰撞、预览、取消、调整、拆除、返还，核心 + 中继二值电网，schema 4、旧档迁移、素材批次与五个实现包均已定义；L3-A 工业地板、L3-B 电力中继与 L3-C 四向传送带已归一入库，继续做 L3-D / E 素材硬闸门，不写占位实现。
...
1. 默认按独立生成会话推进 L3-D 反应器四向端口和 L3-E 储物箱四向端口；萝卜SAMA明确点名切换类别时可在原会话按独立批次与首轮计数继续，逐类完成目标尺寸、共享色板与工业地板实景审阅。
2. L3-D / E 全部过审后从包 1 开始，把采集器专用 `carrying_collector` / 固定 2×2 查询泛化为建筑定义、占用索引、放置控制器和通用实例。
