...
- 回传上线后的下一包深段内容已接入：`S5` 现在会把玩家从基地相位回投台直接带到裂相碎屑、透镜胚片、中继调谐镜和裂相尖塔这条新链上。
...
1. 默认从 `S5` 起跑，完整走一遍“相位回投台 -> 裂相碎屑 -> 透镜胚片 -> 中继调谐镜 -> 裂相尖塔 -> 内层故障轨迹”，确认新收益、风险和基地回投节奏是否对齐。
2. 若只差关键资源或生命 / 防护，直接使用调试 HUD 的 GM 区补状态，不默认回头重打整条链；`S4` 继续用于第二轮深段线前半段，`S5` 用于回传后新包观察。
3. 若这条新链稳定，下一轮主线就以内层故障轨迹为锚继续推进更内层目标；短期仍不新增第三台设备或复杂基地系统。
