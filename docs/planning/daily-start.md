...
1. 继续 `R1` 的“统一光向与整体色调”步骤：先沿正式 `Boot` / `GameRoot` / `VerticalSliceMap` 检查现有 tile、sprite、材质和灯光响应，再确定场景原生的 `DirectionalLight2D` / `CanvasModulate` 接入方式，不生成新素材、不增加程序绘制层。
2. 建立基地到晶体路径的统一左上光向和整体色调基线：基地保持浅暖地面承托暗金属建筑，晶体侧冷色只作区域差异，不让每个 sprite 继续保留互不相干的光感和底色。
3. 从正式入口截取新档开局与修复核心后画面，对比风格锚点和宽幅参考图；若第一轮不成立，只允许一次针对灯光节点、材质响应或色调覆盖根因的修正。通过后再安排接地阴影，不在同一批提前做构图重摆、P2 / P3 或玩法扩展。
