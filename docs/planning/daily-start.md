...
阶段：D 入库与包 1 自动 / 正式入口证据已完成，等待人工复核后进入包 2。

- [首次外勤战斗与关键样本回收](../features/slice-first-field-combat-and-sample-recovery-v1.md) 已完成四张素材入库与包 1；敌人和样本业务留包 2。
...
主目标是人工复核包 1；通过后进入包 2。

1. 从 `/private/tmp/radishcatalyst-combat-package1-review/` 载入“战斗包 1 人工复核”，核对充能后鼠标朝向、左键单击 / 按住 / 松开、`Space` 闪避和前台 UI 互斥。
2. 人工通过后，包 2 只接唯一裂晶爬兽、受击 / 撤离和样本掉落 / 拾取，不提前做交付与 schema 7。
3. 包 2 继续使用已入库敌人、命中和样本素材，不续生成、不重画或复活旧战斗入口。

## 防跑偏规则

- 多世界与 schema 6 已收口；新专题只按明确迁移策略扩展当前切片状态，不复活旧 `SaveService`、旧 `GameRoot` 或旧三槽存档。
- D 与包 1 已完成；人工闸门前不进入包 2。包 2 不提前实现基地交付或 schema 7；旧实现只借规则语义。
