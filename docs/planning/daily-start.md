...
- 过程口径瘦身：入口文档只在包收口更新、冻结新增视觉专题与 checks 增长、每周以 60 秒实机录像为唯一主进度证据。

## 当前事项（P0 素材锚定包）

1. 萝卜SAMA 按 `docs/reference/ai-art-prompts.md` 的"第一批：风格锚点"生成基础反应器 6 个候选，放入 `assets/art-intake/<日期>-batch01/`。
2. 按候选清单下载 1 到 2 个 CC0 素材包到 `assets/third-party/`，作为角色动画补位与整体兜底评估。
3. 请 Claude 审阅最新一批：按视角、光源、色域、剪影和细节密度给出保留 / 重 roll 意见，定稿风格锚点存入 `assets/reference/style-anchor.png`。
4. 锚点定稿前不批量生成第二批之后的资产，也不开始 `P1` 场景改造。
