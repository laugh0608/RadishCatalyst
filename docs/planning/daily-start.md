...
- 状态灯优化后的默认 Metal / 原画质短测 `performance-1789193700-5867` 完成 `90.006909s`，交付 225 件；绘制调用 p95 `508`，模拟步 p95 `1.113ms`，节点处理跨度 p95 `3.324ms`。帧 p95 `21.237ms` 仍超 `16.7ms`，未启动四档长测。
...
- 后续固定同一快照、每段预热 15 秒后生产 45 秒的重复对照已完成。默认 Metal 关闭额外 GPU 计时后帧 p95 `20.595 / 20.853ms`，仍超预算；Vulkan 分项采样明显影响帧时间，分项值不能当默认 Metal 的真实耗时。六段基准原图字节一致，生产 / 保存 / 计时正常，详情见 W37 周志。
- 本机已安装 Instruments `Metal System Trace`；原生采样先后遇到准备页超时、预热失焦，尚未取得有效轨迹。已有完整对照不用重跑；先确认桌面可用再采原生 Metal 执行与显示等待。

## 接续事项（按顺序）

1. **原生 Metal 执行与显示等待归因**：复用本轮已完成的三组重复对照；默认配置仍超预算，不能归咎于额外 GPU 计时。使用已安装的 Instruments 对实际 Godot PID 短时采样，保存有效 `.trace`、时间范围及对应生产段；先区分 GPU 执行与提交 / 显示等待，再做明确修正。Godot 的 Metal GPU 时间戳接口返回零；不继续用该接口或受扰动的 Vulkan 分项差额猜根因。
