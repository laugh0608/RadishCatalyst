...
- 开发复测基线第一版已接入：调试面板可直接载入 `S0` 到 `S5` 六个代码生成的阶段快照，不再默认要求每次从空档打到当前末段。
- 常驻 HUD 已改成分散式常驻布局：左上区域地图、左侧目标卡、右上生存 / 污染状态卡和左下提示 / 日志不再把主线信息强塞进单列。
- 世界相机已改为锁定玩家跟随，深段和最右侧区域会随推进进入视野，不再受静态舞台摆位影响。

## 今日推荐推进

1. 优先用开发复测基线做分层人工复测，不再默认从空档重打到当前末段。
2. 观察前线回传锚点 / 相位回投台时，先从 `S4` 或 `S5` 起跑；只有涉及早期链路、共享任务或存档兼容时，再补 `S0` 全链路复测。
3. 重点观察三件事：基地后勤价值是否还在、基地 -> 深段回投是否足够明确、锚点上线后是否还缺少必要提示或护栏。
