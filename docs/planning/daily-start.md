...
1. 推进 UI baseline 第二包：把主 HUD 信息拆成当前目标、关键资源、基地摘要、角色状态四组，减少状态堆叠感。
2. 先做资源摘要和基地摘要：只显示当前任务相关关键资源、可制造 / 可升级项、缺料和完成后改变。
3. 启动 4 个核心区域的第一轮尺度规划：基地平台、晶体矿脉区、污染边界区、核心稳定站。
4. 为 `region.demo_stabilization_core / 核心稳定站` 起草最小落地方案：入口条件、阶段守卫、核心设备写入和 demo 完成反馈。
5. 若开始改客户端，先做区域 / UI 可读性相关的最小实现，不触碰已冻结的前线行动台、窗口复盘、高压窗口和 `base_action_state`，除非出现 `P0` / `P1`。
