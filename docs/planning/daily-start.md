...
- [游戏本体 UI 视觉定稿 V1](../features/slice-game-ui-visual-finalization-v1.md) 已完成 P0；P1 四组主目标、核心仓库宽面板和损坏世界目录全屏系统均已获批准，只剩首次充能居中确认待目标稿，现有客户端 UI 仍只作为功能基线。
...
11. **UI 定稿 P0、四组 P1 主目标稿、核心仓库宽面板与损坏世界目录已完成。**世界目录以全屏列表 / 详情隔离异常项，损坏项禁用载入与重命名并保留真实回收风险说明；只剩首次充能居中确认，客户端尚未进入视觉接入。
...
2. 核心仓库宽面板与损坏世界目录全屏系统均已获批准；下一张只制作首次充能居中确认，不把此前批准扩展为客户端接入授权。
3. 首次充能获批准后，先写 P2 分面接入顺序与正式入口验证矩阵；在此之前不修改 Theme、场景、控件或资产，也不解冻核心仓库包 2。
