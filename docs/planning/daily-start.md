...
当前为「首版 Demo 体验主干建设：地图承载面拆分第一版」。

首版 Demo 完成规格：[Demo Definition V1](../features/demo-definition-v1.md)。

当前活跃细专题：[Demo Map Surface Decomposition V1](../features/demo-map-surface-decomposition-v1.md)，先拆分地图区域 / gate 承载面，降低 `vertical_slice_map.gd` 行数风险。

最近完成细专题：[Demo Interaction Prompt Surface Decomposition V1](../features/demo-interaction-prompt-surface-decomposition-v1.md)、[Demo Route Return And Base Reentry Readability V1](../features/demo-route-return-and-base-reentry-readability-v1.md)、[Demo Functional Transition Spatial Playability V1](../features/demo-functional-transition-spatial-playability-v1.md)、[Demo Functional Scene Gameplay Density V1](../features/demo-functional-scene-gameplay-density-v1.md) 与 [Demo Supply Pressure Pacing V1](../features/demo-supply-pressure-pacing-v1.md)，2026-06-17 至 2026-06-19 第一包已落地或通过退出判断。
...
3. 读当前活跃细专题 `docs/features/demo-map-surface-decomposition-v1.md`，确认地图区域 / gate 拆分范围、行数预算和检查范围。
4. 只在需要确认上一个阶段边界时，读 `docs/features/demo-interaction-prompt-surface-decomposition-v1.md`、`docs/features/demo-route-return-and-base-reentry-readability-v1.md` 和 `docs/features/demo-functional-transition-spatial-playability-v1.md`。
...
- 当前只收束地图承载面拆分：区域判断、gate 回退和回投位置先从大文件迁出，玩家路线不变。
...
- 必须覆盖真实玩家区域判断、gate 回退和自动检查；不因本轮新增存档 schema。
