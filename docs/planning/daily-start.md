...
重点不再是继续给既有区域追加同类内容包，而是先补试玩门槛级画面与交互可读性，再确认现有 Demo 能否作为一段连续可玩的体验成立。
...
1. 优先做「首版 Demo 试玩门槛级画面与交互可读性第一包」：压低 HUD 黑框压屏，隐藏常驻调试说明，强化地面、路线、设备、资源、敌人和当前交互对象的视觉语义。
2. 第一包完成后再做 `S0` 冷启动试玩或等价完整路线取样；同时用 `S2` / `S21` 复核遗迹外圈、核心稳定站、基地后勤、出发口和复测链。
