...
阶段：三类战斗素材硬闸门已通过，等待独立 D 接入 / 包 1 开工。

- [首次外勤战斗与关键样本回收](../features/slice-first-field-combat-and-sample-recovery-v1.md) C2 已选定并归一 V2 为 `31×30` RGBA；敌人、战斗反馈和关键样本均已审阅通过，尚未接入运行时。
...
- 2026-07-26 包 3：1 / 30 / 损坏 / 回收 159 项断言；暂停返回与创建禁用态两组正式入口各 19 项断言。
- 2026-07-26 萝卜SAMA从保留复核根完成人工路径并确认通过；多世界专题收口。
- 2026-07-27 C1 晶腺关键样本首轮 `3 / 3` 生成完成：V1 建立三叶矿化晶腺身份，V2 消除机械圆口并强化生物样本读法，V3 简化岩片与亮纹以保留小尺寸剪影；三稿、完整 prompt、尺寸与哈希已落盘，等待独立 C2。
- 2026-07-27 C2 选定 V2 `31×30` RGBA：24px 过小、40px 接近敌人体量；V2-32 在败亡敌人、玩家、晶簇及两类货物同场时保持独立生物样本身份。九档机械归一、两张联系表和 7 色 / Alpha / 连通性检查通过。
- 遗留登记：八方向动画（候选专题）、viewport 整数重构（并入 HUD 换皮）、打磨清单观察项。

## 当前下一事项

主目标是按专题边界切换到独立 D 接入 / 包 1 开工；当前 C2 会话不连续进入实现。

1. 清点 A2、B2、C2 三类选定素材与锁定哈希，只复制已审透明产物到运行时资源路径。
2. D 不续生成、不重画、不修改网格 / 色板 / 光向，也不复活旧敌人或旧战斗入口。
3. 包 1 仍从首次充能、玩家工具、输入互斥和 HUD 骨架开工；敌人、样本状态与 schema 7 留后续包。
4. 玩家可见接入按专题要求补真实 `Boot` 有窗口截图和人工闸门；启动 Godot 前先告知萝卜SAMA。

## 防跑偏规则

- 多世界与 schema 6 已收口；新专题只按明确迁移策略扩展当前切片状态，不复活旧 `SaveService`、旧 `GameRoot` 或旧三槽存档。
- A2、B2、C2 均已通过；D 只接已审素材，不续生成。包 1 不提前实现敌人、样本交付或 schema 7；旧实现只借规则语义。
...
- 任务链、立绘对话、八方向动画、viewport 重构与 HUD 换皮；试玩准备与发布。
