...
阶段：第一可玩切片全链串通，包 1 已完成，包 2 待修新手链路理解与 HUD 对比度。
...
主目标是只修包 1 证明会阻断陌生玩家完成全链的两项 `P1`。

1. 从既有状态派生“通电采集 → 反应器供电与双端物流 → 取得 2 催化剂”短目标。
2. 在配方 / 放置面板补当前动作需要的一行规则，不做长教程或通用任务系统。
3. 为现有主要 HUD 增加统一高对比承载；回归全链后由萝卜SAMA计时复核 20 到 40 分钟。
