...
- 当前活跃专题为[试玩反馈整改 V1](../features/slice-playtest-remediation-v1.md)，问题归因、参考 UI 审阅、源码玩家体验审计、五包排期、包 0 与包 1 已完成。
- 包 1 已收口空间阻断、实体物流与 Demo 完成态；当前等待萝卜SAMA确认包 2 五问，不提前修改制造流程、资源数值或装备存档。
...
1. 先为包 2 回答并确认五问，重点冻结制造 / 放置分离、完整材料缺口、采集器 `50`、地板 / 中继 A/B 和最小角色装备页的权威边界。
2. 包 2 确认后实施；装备选择确需持久化时再升级 schema 10，并保证 schema 2—9 确定性迁移。
3. 包 3 统一“仅持有 + 全部平铺 / 分类筛选 + 背包与目标容器拖拽”。
4. 包 4 处理 HUD 安全区、窗口比例、字号 / 明度、核心四状态、设备动画 / 音效和上下文电力叠层。
