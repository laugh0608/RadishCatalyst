...
更新时间：2026-09-13（第二版分段诊断构建完成，窗口待验）
...
整体开关对照与约定时段对照均已完成，官方前后帧 p95 仍出现 `27.186 / 8.584ms` 差异，未建立稳定开销估计。现已把取证收窄到提交后的等待与收尾，第二版隔离诊断引擎构建、状态 62 项、视图 15 项及记录器四种情形检查通过；第二版窗口分段与 fence 身份配对尚未实测。细节见 [Metal 呈现诊断记录](../reference/factory-metal-presentation-review-20260912.md)。
...
1. **验证第二版分段事件**：使用忽略目录 `engine-diagnostic-20260912/postwait-v2/run-postwait.py on`，先告知并等待实际准备页点击，沿用 15 秒预热 / 45 秒生产和双存档根隔离。核对事件零丢弃、各段配对、fence 创建 / 销毁与 command 创建 / 重置身份，再确认原图 / 状态一致；新增标记的开销仍待验证。
2. **按同帧证据定位慢阶段**：已覆盖 fence 等待、待释放资源整理、回调、帧统计及显存统计，原提交顺序保持不变。只在实际慢帧上比较分段与未覆盖余量，避免嵌套重复计时和分位数相减；若只得到快帧则记为未复现，不再追加整体开关批次或索要相同的稳定时段。证据指出具体等待后，再决定是否需要同轮 GPU 关联；不提前改提交策略。
