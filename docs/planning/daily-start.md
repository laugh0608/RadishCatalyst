...
- 当前活跃专题 [多世界存档列表](../features/slice-multi-world-save-list-v1.md)：最多 30 个独立世界、稳定目录、轻量元数据、三份备份、可恢复回收和测试复核列表；第一包推进目录服务与当前单档无损迁移。
...
主目标是推进多世界存档第一包，不修改稳定的 schema 6 玩法字段。

1. 新增 `SliceSaveCatalog` 与世界元数据契约，最多枚举 / 创建 30 个独立世界；`SliceSaveService` 保持单世界职责并升级三份备份。
2. 从当前 `slice_world.json` / `.bak` 做读回校验后的无损导入，旧文件保留；自动检查先覆盖目录、上限、排序、回收恢复、损坏隔离和迁移，不提前接启动 UI。

## 防跑偏规则

- schema 6 是稳定世界状态输入；多世界只增加外层目录、元数据和选择，不借机改玩法字段或复活旧 `SaveService`。
