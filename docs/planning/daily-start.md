...
- 当前活跃专题为[试玩反馈整改 V1](../features/slice-playtest-remediation-v1.md)，问题归因、参考 UI 审阅、源码玩家体验审计、五包排期与包 0 已完成。
- 萝卜SAMA已于 2026-08-25 确认包 0；空间与保存合同、保存调度职责、专项和整改前实机基线已经收口，下一步是包 1 开工闸门。
...
先为包 1 回答玩家结果、实现介质、成功证据、失败判据和文档落点，再由萝卜SAMA确认范围；不直接从 UI 换色或批量动画开工：

1. 包 1：修复 Demo 完成态、传送带通行与排序、反应器和设备边界、采集器真实物流，并实机观察连续射击保存节奏。
2. 包 2：修复制造停留、完整材料缺口、采集器 `50`、地板 / 中继数值和最小角色装备页。
