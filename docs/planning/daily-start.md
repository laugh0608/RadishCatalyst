...
- 上述更改已提交 `cff8b0ef`。随后窗口补验取得长帧、三次规模拆改、四侧边缘移动 / HUD 原图，以及系统鼠标打开二次确认并取消的证据；整轮仍因弹窗注入、操作等待超时和后续启动失焦未通过，原生重试 / 最终放弃仍待补验。
...
1. **补齐短窗口复验**：从隔离正式 Boot 补原生“重试保存”、最终确认放弃与完整返回路径。计时、暂停恢复、二次确认取消已有部分证据，不因后台长测通过而关闭剩余窗口验收项；需要占用桌面时另行安排。
2. **核对玩家路径缺口**：规模连续拆改、地图四侧边缘移动 / 相机 / HUD 已有窗口原图；按原有合同逐项补缺，保留失败批次记录，不把信号注入当鼠标命中或亲测。
