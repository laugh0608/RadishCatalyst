...
- 2026-06-21 已基于多轮定位截图继续完成晶体 / 污染主次治理：2 号晶体点位的旧蓝底、蓝色大 cue、横向路线和出发口对象 / 静态标签进一步退场；3 号污染边界的跨区残骸标签、蓝色长路线和旧黄绿底板继续降权。新截图复核后，当前转向第一条工业路径横切；首段阶段反馈已从多色叠加改成背景降权、局部工作面、低饱和轮廓和单主信号。根据新档首屏截图反馈，前哨核心恢复前改为专用核心恢复视图，完整设备层、路线 cue、侧边设备 marker 和敌人暂不进首屏；恢复后资源段已补手持采样点、采集器工作面、出料托盘和回基地装车端口。
...
- 当前不切换专题；优先推进 `基地出发 -> 手持采样 -> 采集设备自动开采 -> 基地收料 -> 基础反应器 -> 仓储输出 -> 出发整备收益` 的第一段可跑画面和操作阶段反馈，前哨核心恢复前只显示核心恢复视图，后续路径层、完整设备层和敌人不进首屏；恢复后局部设备工作面必须比全图规划层更醒目。
...
- 若第一工业路径仍像流程图，继续减少全图规划层权重并补局部设备 / 地貌 / 输出工作面；若当前读法成立，再推进采集设备建造 / 产出接线、设备状态灯、资源 / 浆液流动、敌压警告和核心写入反馈。
