...
- 当前活跃专题 [多世界存档列表](../features/slice-multi-world-save-list-v1.md)：目录、列表、游戏内返回及动作互斥自动证据已完成；等待人工收口。
...
主目标是复核列表动作互斥、暂停、保存返回与世界切换。

1. 从真实列表选择已有世界，确认“创建并进入”禁用而载入仍可用。
2. 载入世界后按 `Esc` 检查暂停、设置与确认面板。
3. 保存返回后切换世界并重载原世界；确认后收口。
