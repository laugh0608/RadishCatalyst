...
9 月 13 日已完成获授权的隔离引擎构建与五次同状态窗口对照。事件验证支持“获取 drawable 后才进行本帧图编码 / 提交”，但不能证明这是长帧主因；重复官方与标记关闭组波动明显，标记开销尚未单独量清。构建后目录占用约 8.23GiB，详细数据见呈现诊断记录。
...
1. **先稳定对照并量清标记开销**：构建、记录器四种情形、状态 62 项和视图 15 项检查通过；五组基准状态 / 原图一致。官方帧 p95 从 17.348 变为 8.549ms，标记关闭从 30.897 变为 8.585ms，开启组为 40.358ms；不将组间差额直接归因于构建或标记。后续先记录运行条件并做受控开关对照，稳定性不足时先复盘，不盲试参数。
2. **开销明确后再关联 GPU**：本轮 2,194 个生产帧已核实获取后编码 / 提交，但未取得同轮原生 GPU 轨迹，也未证明可获益的队列空隙；暂不修改提交策略。后续采样仍先告知、等待实际点击准备页，并隔离双存档根；只有时钟与身份对齐的数据支持收益时，才提出具体补丁及同步风险范围。
