...
- 2026-07-07 完成 `R0` 代码基线：正式 `Boot` 入口可见，首分钟旧层与过早设备显隐收束，修复核心后出现晶体路径视觉，晶体任务当前目标选择遵守朝向规则。

## 明天事项（2026-07-08，首分钟实机复核）

1. 沿正式 `Boot` / `GameRoot` / `VerticalSliceMap` 链路跑新档第一分钟截图序列，不用临时场景或临时入口。
2. 截图至少覆盖开局损坏核心、修复核心、晶体路径出现、靠近晶体采集提示；额外确认背向晶体时不提前弹提示。
3. 若截图序列不成立，优先修场景显隐、晶体路径可读性、交互范围或 HUD 当前目标，不恢复 `P2` 录像。
4. 若截图序列成立，再恢复 60 秒路径录像复判；录像成立后准备 `P3` 旧层清除收口。
