...
## 今日事项（2026-07-09，`R1` 地基包开工）

1. 先建连续地形层：用 `client/assets/tiles/demo_presentation_rebuild/` 既有 tile，`TileMapLayer` 铺满相机视野替换近黑空背景，先出一张铺满地形的实机截图。
2. 地形成立后再叠基地平台落座面、统一光向 / 色调、物件接地阴影，最后做构图收拢；地基不成立不摆物件。
3. 每步截图对比 `assets/reference/style-anchor.png` 与宽幅参考图，按"场景成立四要件"列差距；同一步最多重试一次，仍不成立升级为素材 / 介质决策。
4. 旧视觉层继续硬禁用，本包不删文件、不新建检查；`R0` 首分钟流程读法逻辑保留复用。
