...
- 当前活跃专题 [多世界存档列表](../features/slice-multi-world-save-list-v1.md)：30 世界目录、稳定 ID、元数据、三份备份、回收恢复和当前单档无损迁移已完成；下一包接启动菜单列表与世界选择。
...
主目标是推进多世界存档包 2，不修改稳定的 schema 6 玩法字段。

1. `StartupMenu` 增加可滚动世界列表、详情、命名输入、载入、重命名、回收 / 恢复和上限反馈，不创建 30 个固定按钮。
2. `Boot` 在创建或选择世界后注入对应单世界服务；正式入口证明两个世界互不串档、新游戏不覆盖和损坏项不阻断正常世界。
