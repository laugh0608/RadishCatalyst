...
- 当前活跃专题 [L4 传送带物流](../features/slice-conveyor-logistics-v1.md)：包 1 直线物流与包 2 转角 / 端点拓扑证据已完成，下一步包 3 合流与人工收口。
...
1. 推进 L4 包 3：离线派生双 / 三路合流固定帧，不运行时画轨道或旋转 sprite。
2. 接入稳定 `instance_id` 轮询、公平性、满箱回压与重启续跑，按正式 `Boot` + 隔离存档 + 窗口截图完成人工收口；暂不追加 L3 同层画面微调。
