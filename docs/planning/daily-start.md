...
- 2026-07-12 `R1` 统一光向与整体色调已通过新档开局 / 修复核心后截图验收：场景复用贴图烘焙左上光，以 `CanvasModulate` 建立中性日光基线，并归一活跃 sprite 的逐件 RGB 染色；下一步进入接地阴影。
...
1. 继续 `R1` 的接地阴影步骤：只处理前哨核心、玩家、基地平台设备和晶体路径关键物件的落座关系，不同时重摆构图。
2. 阴影介质必须是场景原生 `Sprite2D` / 贴图轮廓投影或等价材质响应；不新增程序绘制视觉层，不为阴影重生成整套素材。
3. 仍从正式入口截取新档开局与修复核心后画面，确认核心、角色和设备不再悬浮；同类方案最多复核两轮，通过后才进入构图收拢。
