...
- 当前活跃专题 [L5 反应器接入自动化](../features/slice-reactor-automation-v1.md)：催化剂货物、加工 overlay 与包 1–2 的状态机、schema 6、机器物流及操作面已完成，下一步进入包 3。
...
1. 开工 L5 包 3：从真实 `Boot → 新游戏` 使用自然晶体、合成、放置与储物操作搭出“晶体箱 → 入口带 → 通电反应器 → 出口带 → 催化剂箱”，不得直接注入最终世界状态替代玩家路径。
2. 使用隔离存档分别验证加工中与待出料重启续跑，保留完整产线、催化剂流动和目标箱画面；自动、正式入口、截图和萝卜SAMA人工复核全部通过后才收口 L5 与配方加工 arc。
