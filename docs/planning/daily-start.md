...
- [首次外勤战斗与关键样本回收](../features/slice-first-field-combat-and-sample-recovery-v1.md) B2 已选定并归一 V2；等待独立 C1，不进入实现。
...
主目标是完成独立 C1 晶腺关键样本首轮生成，不进入 C2 或玩法实现。

1. 开工前重读当前专题、像素规范和提示词口径；使用新批次、独立 manifest 与 `0 / 3` 计数。
2. 只生成晶腺关键样本的掉落 / 可拾取身份候选，不混敌人、战斗反馈、UI、场景或基地交付画面。
3. 每次只生成 1 张并立即落盘、记录完整 prompt / 尺寸 / 哈希；达到 `3 / 3` 后停手。
4. C2 审阅 / 归一须另行明确切换阶段；C1 不去底、不裁切、不拼联系表、不接 Godot 或玩法代码。

## 防跑偏规则

- 多世界与 schema 6 已收口；新专题只按明确迁移策略扩展当前切片状态，不复活旧 `SaveService`、旧 `GameRoot` 或旧三槽存档。
- B2 已选定 V2 并停止；C1 只生成关键样本，不审阅、不接入代码；旧实现只借规则语义。
