...
主目标是进入包 3，完成基地交付、schema 7 与完整闭环。

1. 升级 schema 7，严格保存玩家生命与五态遭遇，覆盖旧档迁移、备份回退和多世界隔离。
2. 接入核心交付优先级、抗蚀内衬与 `100 → 120` 永久生命收益，并让战斗关键事件自动保存。
3. 跑重启矩阵，再用真实 `Boot` 完成充能至交付全链与截图复核。

## 防跑偏规则

- 多世界与 schema 6 已收口；新专题只按明确迁移策略扩展当前切片状态，不复活旧 `SaveService`、旧 `GameRoot` 或旧三槽存档。
- 包 3 只完成既定交付、持久化和闭环，不扩敌种、技能树、装备或通用任务系统；旧实现只借规则语义。
