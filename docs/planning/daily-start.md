...
1. **审计实际引擎的获取 / 提交 / 释放顺序**：既有轨迹离线核对已收口，详见 [Metal 呈现诊断记录](../reference/factory-metal-presentation-review-20260912.md)。581 对等待可验证但无对象标识，不能从现有数据证明回收唤醒因果；不重复同类导出或盲试限帧。按采集记录的 Godot 构建 hash 只读核对 Metal 调用链，输出具体文件 / 函数、可证伪假设和必要补测位置，再判断项目层修正或引擎级取证范围。暂不下载整仓、安装依赖、构建引擎或修改驱动、同步、缓冲数量与画质。
