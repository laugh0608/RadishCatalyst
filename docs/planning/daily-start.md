...
9 月 25 日第二版离线分析与首轮窗口分段完成，随后正式 Godot 原配置短测也通过（帧 p95 `8.680ms`）。四档长测首档在约 220.7 秒收到 `focus_lost` 后停止，完成档位为 0，未自动重开；完整 Shell 客户端无窗口回归随后通过。四档连续前台证据仍待补，但按萝卜SAMA要求等待明确通知后再启动；期间推进首包范围内不占用前台的任务，不能把中断样本计作长测通过。批次及字体错误复盘见 [W39 周志](../devlogs/2026-W39.md)，分段分析边界见 [Metal 呈现诊断记录](../reference/factory-metal-presentation-review-20260912.md)。
...
- 正式版本短测批次 `official-20260925-135157` 完成 45 秒生产，8 项检查通过，帧 p95 `8.680ms`；原图与本日诊断图字节一致。长测 `performance-1790315800-15236` 保留 220.697779 秒、26,411 帧、7 次成功自动保存，但 `phases=[]`，不计验收。
...
本轮前台长测因失焦停止，不自动重开；短测、分析器与本机无窗口回归已有可复用证据。

1. **补齐四档前台长测**：仅在萝卜SAMA明确通知开始后安排约 30 分钟连续前台时段，启动前先告知，由准备页实际点击 `foreground-performance-review.gd` 入口；使用正式版本、`--active --phase-seconds=450`，1080 / 最大化、局部 / 拉远各 450 秒。本轮首档不足 450 秒，从头执行，不续接或拼接部分样本；失焦、关闭或档位预算失败即停。
2. **依实际结果收口**：保留完整帧、模拟步、自动保存、计时守恒与积压数据，实际审阅各档原图。若再次出现持续慢帧，复用第二版分析器定位同帧等待与收尾；不追加整套开关对照，不盲试画质、限帧或提交策略。快样本不能排除历史慢路径。
3. **复用已测边界**：本轮正式版本原配置短测、74 个 Godot 入口（含导入）的 Shell 回归以及已有原生关闭 / 重进证据可复用；代码或环境改变后按影响补测。此前未出现的 fence 地址复用、初始值 0 和 frame 0 回调仍仅有离线样本覆盖。
4. **补齐界面缺口后交付亲测**：按首包清单核对备份 / 坏档反馈、遗留锁恢复及旧二维载入画面；已有原生保存失败、关闭 / 重进证据可复用。窗口安排另行告知，当前不启动；Windows / PowerShell 和目标硬件仍单列待验，首包整体验收尚未完成，不改变退出条件。
