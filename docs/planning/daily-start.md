...
1. **验证提交节奏与 drawable 等待**：复用已取得的原生 Metal 轨迹，不重做整轮采样。下一对照保持原画质、像素、VSync 和同一生产快照，仅在测试进程比较原不限帧 / 临时 `Engine.max_fps=60` / 恢复三段；每段预热 15 秒、生产 45 秒，不启用额外 GPU 计时。忽略目录 `gpu-breakdown-20260912/frame-cap.gd` 已准备，尚未运行；保留完整帧、保存和恢复设置证据。主动限帧能否减少等待仍是假设，不能直接改正式配置或以采样内 GPU p95 代替整帧预算。
