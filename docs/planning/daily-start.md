...
- 当前不切换专题；优先推进 `基地出发 -> 手持采样 -> 采集设备自动开采 -> 基地收料 -> 基础反应器 -> 仓储输出 -> 出发整备收益` 的第一段可跑画面和操作阶段反馈，前哨核心恢复前只显示核心恢复视图；3 / 4 号截图点必须分别读成采集器出料和基地交接，局部设备工作面必须比全图规划层更醒目。
...
- 先用控制面板载入 1 / 3 / 4 号截图点：1 号核对新档首屏是否只读前哨核心，3 号核对采集器自动出料，4 号核对基地收料、反应器进料、产物入库和整备交接。
- 截图复核只看工业路径是否被局部设备工作面带动：若仍像流程图，继续削弱全图规划层，并补采集器托盘、基地收料口、反应器输入台、储存输出位和整备交接口的轮廓与材质。
- 若 1 / 3 / 4 号读法成立，下一包推进设备状态灯、资源 / 浆液流动、敌压警告和核心写入反馈，把同一套视觉语言延伸到污染链与核心稳定站。
