...
- 当前不切换专题；第一工业路径通过后，优先复核污染链与核心稳定站是否能读出设备状态、物料输入输出、敌压解除和核心写入 / 复测反馈。
...
- 先复核污染边界：确认污染沉积、过滤器、药剂输出、浆液输出和回收 / 核心准备端口能通过状态灯、物料槽和短流带读出阶段。
- 再复核核心稳定站：确认恢复补给、守卫压力、回写缓存、核心写入、复测读数和物流回流能通过同一套状态反馈读出阶段。
- 若污染链或核心稳定站仍像流程图，优先补局部设备轮廓、危险边界和状态反馈，不回到长箭头和大标签。
