...
- 原生 Metal 轨迹已取得：用户点击准备页后连续生产 90 秒，无中断；帧 p95 `21.227ms` 仍超预算。10.906 秒轨迹内 drawable 等待 p95 `19.225ms`，按帧合并 GPU 活动区间 p95 `15.073ms`；二者重叠，不相加或相减。原生采样缺口已关闭，继续验证提交节奏。

- 同状态原不限帧 / 临时 60 FPS / 恢复对照已完成，帧 p95 `20.131 / 20.772 / 18.104ms`；三段均超预算，临时限帧未改善。生产、保存、画面和设置恢复正常，不将 60 FPS 上限写入正式配置；首次中断已由用户确认为误切换窗口。

## 接续事项（按顺序）

1. **确认隔离引擎诊断构建范围**：实际构建源码审计已完成。正常路径先获取 drawable，帧末才编码 / 提交渲染图；4.7.2 的三个相关文件相同，项目层没有确定修复点。下一项为 [隔离诊断构建方案](../reference/factory-metal-presentation-review-20260912.md)：给获取、图编码、commit 和 present 增加可关闭标记，先验证顺序假设，不修改提交策略。方案涉及完整源码下载、专用虚拟环境安装 SCons 和独立构建，等待萝卜SAMA授权后实施；不替换系统 Godot，窗口验证另先告知。
