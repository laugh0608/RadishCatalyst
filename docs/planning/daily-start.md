...
当前为「首版 Demo 体验主干建设：运行时承载面拆分第一版」。

首版 Demo 完成规格：[Demo Definition V1](../features/demo-definition-v1.md)。

当前执行细专题：[Demo Runtime Surface Decomposition V1](../features/demo-runtime-surface-decomposition-v1.md)，覆盖主路径连续性第一包落地后的运行时检查承载面拆分。

最近完成细专题：[Demo Main Path Continuity V1](../features/demo-main-path-continuity-v1.md)、[Demo Save State Contract V1](../features/demo-save-state-contract-v1.md) 与 [Demo Resource Chain State V1](../features/demo-resource-chain-state-v1.md)，2026-06-16 第一包已落地。
...
重点是推进运行时承载面拆分第一包；不再继续给主路径连续性、存档契约、资源链状态、场景身份、路线支撑、工具打击校准、防护响应、主线完成感、既有战斗 / 工业 / 核心场景读法点追加同类内容包，也不把纯基线复核写成下一步主线。
...
3. 读当前执行细专题 `docs/features/demo-runtime-surface-decomposition-v1.md`，确认本轮拆分范围。
4. 读最近完成细专题 `docs/features/demo-main-path-continuity-v1.md`，确认不要继续加厚主路径连续性同一检查包。
...
- 当前推进 Demo 运行时承载面拆分：先把独立提示 / 运行时断言从接近硬上限的大文件拆出，并接入 `check-client`。
