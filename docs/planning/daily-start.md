...
阶段：第一可玩切片全链串通，包 2 人工计时达标，但体验反馈尚未完成归因与修正范围确认。
...
主目标是把人工反馈归因并确定修正包边界，不直接进入朋友盲测。

1. 当前 `P1` 候选：降低等待主导感、缩减 HUD 视野占用、鼠标联动放置、电网范围预览和设备地板需求反馈。
2. 纯文字建造 / 背包面板先列 `P2` UI 专题候选，不与放置操作一次性混改。
3. 3/4 俯视体积感不足是介质与风格锚点升级点，必须由萝卜SAMA另行决定路线。

## 明日事项（2026-07-28）

1. 开工前先更新全链专题的决策闸门，把“节奏 / HUD”和“鼠标放置 / 空间反馈”拆成两个可独立验收的小包。
2. 用真实新档记录有限晶体、可再生采集、合成、放置和纯等待的时间分布，再决定调整采集间隔、资源预算还是交互数量；不先拍脑袋改配方。
3. 先设计紧凑 / 按需展开 HUD；再定义鼠标网格联动、中继连接与设备供电范围、设备足印 / 地板缺口预览，以及连续铺地的交互取舍。
4. 纯文字建造 / 背包图形化另立 UI 候选；3/4 体积感单独做架构级视觉路线复核。两者都不塞进当前 `P1` 修正包，修正前不进入包 3 盲测。

## 防跑偏规则

- 多世界与 schema 7 已收口；新专题只按明确迁移策略扩展当前切片状态，不复活旧 `SaveService`、旧 `GameRoot` 或旧三槽存档。
- 包 2 只修产线短目标与 HUD 对比度；不新增存档字段，不复活旧通用任务链，不借机改资源配方。
...
- 包 2 内部全链和人工计时通过前的朋友盲测、试玩分发准备与发布。
