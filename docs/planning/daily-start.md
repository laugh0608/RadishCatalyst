...
阶段：包 2 人工反馈已修正，等待复核确认。

- [首次外勤战斗与关键样本回收](../features/slice-first-field-combat-and-sample-recovery-v1.md) 已完成唯一敌人、撤离与样本回收；基地交付和 schema 7 留包 3。
...
- 2026-07-27 包 1 人工通过；包 2 接入唯一敌人、撤离与样本。首次复核发现缺字和阴影偏移；已内置字体并按单帧高度修正。专项 53 项、`Boot` 56 项、修正 10 项通过。
- 遗留登记：八方向动画（候选专题）、viewport 整数重构（并入 HUD 换皮）、打磨清单观察项。

## 当前下一事项

主目标是复核字体、阴影与包2路径；确认无阻碍后才进入包 3。

1. 启动页核对“战斗包 2 人工复核 · 唯一敌人”无缺字；载入后检查敌人阴影贴脚、预警、伤害、撤离重试和敌人回满。
2. 击败后确认只掉落一份晶腺样本，靠近按 `E` 后目标切为已回收且普通背包不新增任务物。
3. 人工通过后再接基地交付、抗蚀内衬、schema 7 和重启矩阵。

## 防跑偏规则

- 多世界与 schema 6 已收口；新专题只按明确迁移策略扩展当前切片状态，不复活旧 `SaveService`、旧 `GameRoot` 或旧三槽存档。
- 包 2 人工确认前不进入基地交付或 schema 7；旧实现只借规则语义。
