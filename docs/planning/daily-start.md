...
1. **继续离线核对显示等待链路**：原限帧对照已完成且无改善，不重复同一试验或继续盲试限帧值。复用原生 Metal 轨迹，按同一 command-buffer ID 已关联 581 次 present 请求与完成，间隔 p95 `15.775ms`；该值含排队与 GPU 工作，不等于屏幕显示延迟。接着核实同一 drawable 的呈现 / 回收与 `nextDrawable` 等待边界，先确认标识和时钟语义，再判断是否存在可修正的提交或合成问题。现有证据尚不足以永久更改驱动、同步、缓冲数量或画质。
