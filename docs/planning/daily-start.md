...
- 2026-07-12 `R1` 统一光向、整体色调与接地阴影均已通过新档开局 / 修复核心后截图验收：场景复用贴图烘焙左上光，以 `CanvasModulate` 建立中性日光基线；`GroundShadow` 组件复用目标 sprite 轮廓并向右下压缩投影，核心、玩家和修复后关键物件已明确落座。
...
1. 继续 `R1` 第 5 步构图收拢：只处理焦点、疏密和前中后景关系，不回头调地形 atlas、逐件色调或接地阴影参数。
2. 保留核心、玩家、基地设备和晶体路径关键物件的同向接地关系；构图调整不得让阴影遮挡平台纹理、能量灯或交互反馈。
3. 仍从正式入口截取新档开局与修复核心后画面，确认基地首屏满足场景成立四要件；同类方案最多复核两轮，通过后再复判 `P2`。
