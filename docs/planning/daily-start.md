...
当前为「首版 Demo 可试玩呈现建设与完整取样验收」。

重点不再是继续给既有区域追加同类内容包；当前要把现有原型整理成可试玩、可录屏、可对外取反馈的一段完整体验。
...
- 2026-06-14 阶段口径复核结论：当前内容建设没有偏离长期主线，但同一条核心站归档后回访链继续加厚的收益递减；阶段应转入可试玩呈现建设与完整取样验收。
- 首版 demo 12 区域封顶；第 12 区域是 `region.demo_stabilization_core / 核心稳定站`。

## 下一步重点

1. 优先做「首版 Demo 可试玩呈现建设包」：以 `S0` 完整路线为主轴，统一基地、晶体区、污染边界、遗迹外圈和核心稳定站的画面层级、路线读法、交互焦点、战斗反馈和回基地理由。
2. `S2` / `S21` 作为后段定位和验收辅助，确保遗迹外圈、核心稳定站、基地后勤和完整路线使用同一套呈现目标。
3. 只修真正影响整段试玩体验的问题：主线卡死、关键资源断档、坏档、任务无法完成、UI 完全无法判断下一步、重复无决策空跑和关键战斗反馈不足。
...
- 不把完整发布准备、大规模 polish 或静态审计作为当前阶段目标；当前优先整段可试玩呈现、完整取样验收和工程边界。
