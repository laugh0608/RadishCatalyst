...
## 明天事项（2026-07-06，P2 三区证据收口）

1. 用修正后的截图定位重新补晶体矿脉、污染边界、核心稳定站三张定点实机截图，并对比 `assets/reference/style-anchor.png` 记录具体差距。
2. 若某张截图失败，只收束对应 `DemoPresentationFieldZones` / `DemoPresentationCoreStation` 的旧层遮挡、摆位、范围和可读性；不生成新素材，不改玩法、任务、存档或数据表。
3. 三张截图成立后录制第一段 60 秒实机录像，作为本周主进度证据。
4. 录像成立后进入 `P3` 旧层清除收口包：按专题清单删除旧视觉层脚本 / 节点，并同步修剪失效检查。
