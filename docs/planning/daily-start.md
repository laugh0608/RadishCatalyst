...
- 2026-07-07 确认正式 Godot 主入口不能是编辑器空壳；`Boot` 保留为正式运行入口，但打开工程应直接看到正式启动内容，不用临时工作台场景绕开项目本体。

## 今日事项（2026-07-07，首分钟体验重基线）

1. 保持 `Boot` / `GameRoot` 为正式入口链路，让 Godot 打开主场景能看到正式启动内容。
