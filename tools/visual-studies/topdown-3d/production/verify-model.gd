...
func _run() -> void:
	...
	for scenario in data.scenarios:
		...
		for step in scenario.steps:
			...
			var mismatch := _compare(step.expected, actual, "state")
			checkpoints += 1
	...
	var file := FileAccess.open(record_root.path_join("model-result.json"), FileAccess.WRITE)
	if file == null:
		failures.append("Unable to write model result")
	else:
		file.store_string(JSON.stringify({"checkpoints": checkpoints, "scenarios": data.scenarios.size(), "failures": failures,
			"scope": "Web oracle replay, material/kit conservation, 8 curve directions; test-only injected capacity and merge fixtures"}, "\t") + "\n")
		file.close()
	print("Godot production model: %d checkpoints, %d failures" % [checkpoints, failures.size()])
...
func _snapshot(model: RefCounted, actor: Dictionary, result: Dictionary) -> Dictionary:
	...
	return {"entities": entities, "kits": model.kits, "bag": model.bag, "next_id": model.next_id,
		...
		"delivered_batch": model.delivered_batch, "lesson": model.lesson,
