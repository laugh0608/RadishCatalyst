...
const ASSET_ROOT := "res://production/assets/"
...
func _ready() -> void:
	for name in ["yard", "ore", "collector", "reactor", "storage", "engineer", "cargo",
		...
		assert(assets[name] is PackedScene, "Missing shared mesh: " + name)
	add_child(assets.yard.instantiate())
	add_child(assets.ore.instantiate())
	...
	sync_camera()
	for node in [grid, guides, ghost, selection]:
		add_child(node)
	var grid_mat := Parts.material("89958a", 0, 1)
	for x in range(-10, 11):
		_overlay_box(grid, Vector3(x, 0.035, 0.5), Vector3(0.014, 0.006, 13), grid_mat)
	for z in range(-6, 8):
		_overlay_box(grid, Vector3(0, 0.035, z), Vector3(20, 0.006, 0.014), grid_mat)
	var guide_mat := Parts.material("c4c69a", 0, 0.8)
	for type in ["collector", "reactor", "storage"]:
		var cell: Vector2i = Model.REFERENCE[type]
		footprint(guides, cell, Model.CATALOG[type], guide_mat)
...
func draw(model: RefCounted, actor: Dictionary, ui: Dictionary) -> void:
	...
	guides.visible = ui.build and ui.guides
