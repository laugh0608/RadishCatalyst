...
$checkScript = Join-Path $clientRoot "scripts/checks/slice_first_playable_journey_check.gd"
if (-not (Test-Path -LiteralPath $checkScript -PathType Leaf)) {
    Write-Error "Slice first playable journey check script not found: ${checkScript}"
    exit 1
}

$runId = "slice-first-playable-journey-{0}-{1}" -f $PID, [DateTime]::UtcNow.ToString("yyyyMMddHHmmssfff")
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        ...
        Write-Error "Godot import before first playable journey check failed with exit code ${LASTEXITCODE}."
        exit $LASTEXITCODE
    }
    ...
    if ($LASTEXITCODE -ne 0) {
        ...
        Write-Error "First playable journey check failed with exit code ${LASTEXITCODE}."
        exit $LASTEXITCODE
    }
    ...
    if ($unexpectedErrors.Count -gt 0) {
        ...
        Write-Error "First playable journey check reported unexpected errors."
        exit 1
    }
}
finally {
...
Write-Host "First playable journey checks passed."
