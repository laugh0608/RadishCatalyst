...
$checkScript = Join-Path $clientRoot "scripts/checks/slice_playtest_remediation_package4_check.gd"
if (-not (Test-Path -LiteralPath $checkScript -PathType Leaf)) {
    Write-Error "Slice playtest remediation package 4 check script not found: ${checkScript}"
    exit 1
}

$runId = "slice-playtest-remediation-package4-{0}-{1}" -f $PID, [DateTime]::UtcNow.ToString("yyyyMMddHHmmssfff")
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        ...
        Write-Error "Godot import before package 4 check failed with exit code ${LASTEXITCODE}."
        exit $LASTEXITCODE
    }
    ...
    if ($LASTEXITCODE -ne 0) {
        ...
        Write-Error "Slice playtest remediation package 4 check failed with exit code ${LASTEXITCODE}."
        exit $LASTEXITCODE
    }
    ...
    if ($unexpectedErrors.Count -gt 0) {
        ...
        Write-Error "Slice playtest remediation package 4 check reported unexpected errors."
        exit 1
    }
}
finally {
...
Write-Host "Slice playtest remediation package 4 checks passed."
