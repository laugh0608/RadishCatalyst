...
$checkScript = Join-Path $clientRoot "scripts/checks/slice_logistics_check.gd"
if (-not (Test-Path -LiteralPath $checkScript -PathType Leaf)) {
    Write-Error "Slice logistics check script not found: ${checkScript}"
    exit 1
}

$runId = "slice-logistics-{0}-{1}" -f $PID, [DateTime]::UtcNow.ToString("yyyyMMddHHmmssfff")
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        ...
        Write-Error "Godot import before slice logistics check failed with exit code ${LASTEXITCODE}."
        exit $LASTEXITCODE
    }
    ...
    if ($LASTEXITCODE -ne 0) {
        ...
        Write-Error "Slice logistics check failed with exit code ${LASTEXITCODE}."
        exit $LASTEXITCODE
    }
    ...
    if ($unexpectedErrors.Count -gt 0) {
        ...
        Write-Error "Slice logistics check reported unexpected errors."
        exit 1
    }
}
finally {
...
Write-Host "Slice logistics checks passed."
