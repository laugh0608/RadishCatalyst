...
$checkScript = Join-Path $clientRoot "scripts/checks/slice_inventory_model_check.gd"
if (-not (Test-Path -LiteralPath $checkScript -PathType Leaf)) {
    Write-Error "Slice inventory model check script not found: ${checkScript}"
    exit 1
}

$runId = "slice-inventory-model-{0}-{1}" -f $PID, [DateTime]::UtcNow.ToString("yyyyMMddHHmmssfff")
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        ...
        Write-Error "Godot import before slice inventory model check failed with exit code ${LASTEXITCODE}."
        exit $LASTEXITCODE
    }
    ...
    if ($LASTEXITCODE -ne 0) {
        ...
        Write-Error "Slice inventory model check failed with exit code ${LASTEXITCODE}."
        exit $LASTEXITCODE
    }
    ...
    if ($unexpectedErrors.Count -gt 0) {
        ...
        Write-Error "Slice inventory model check reported unexpected errors."
        exit 1
    }
}
finally {
...
Write-Host "Slice inventory model checks passed."
