...
$checkScript = Join-Path $clientRoot "scripts/checks/slice_playtest_remediation_package4b_check.gd"
if (-not (Test-Path -LiteralPath $checkScript -PathType Leaf)) {
    Write-Error "Slice playtest remediation package 4B check script not found: ${checkScript}"
    exit 1
}

$runId = "slice-playtest-remediation-package4b-{0}-{1}" -f $PID, [DateTime]::UtcNow.ToString("yyyyMMddHHmmssfff")
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        ...
        Write-Error "Godot import before package 4B check failed with exit code ${LASTEXITCODE}."
        exit $LASTEXITCODE
    }
    ...
    if ($LASTEXITCODE -ne 0) {
        ...
        Write-Error "Slice playtest remediation package 4B check failed with exit code ${LASTEXITCODE}."
        exit $LASTEXITCODE
    }
    ...
    if ($unexpectedErrors.Count -gt 0) {
        ...
        Write-Error "Slice playtest remediation package 4B check reported unexpected errors."
        exit 1
    }
}
finally {
...
Write-Host "Slice playtest remediation package 4B checks passed."
