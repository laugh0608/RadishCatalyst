...
try {
    ...
    $godotOutput = & $GodotExe --headless --path $clientRoot --import --quit --no-header 2>&1
    if ($LASTEXITCODE -ne 0) {
        $godotOutput | ForEach-Object { [Console]::Error.WriteLine($_) }
        Write-Error "Godot client check failed with exit code ${LASTEXITCODE}."
        exit $LASTEXITCODE
    }
    ...
    if ($unexpectedErrors.Count -gt 0) {
        ...
        Write-Error "Godot client check reported unexpected errors."
        exit 1
    }
}
finally {
...
Write-Host "Godot client project imported."
