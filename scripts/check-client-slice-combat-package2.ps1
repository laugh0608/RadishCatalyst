...
$checkScript = Join-Path $clientRoot "scripts/checks/slice_combat_package2_check.gd"
if (-not (Test-Path -LiteralPath $checkScript -PathType Leaf)) {
    Write-Error "Slice combat package 2 check script not found: ${checkScript}"
    exit 1
}

$runId = "slice-combat-package2-{0}-{1}" -f $PID, [DateTime]::UtcNow.ToString("yyyyMMddHHmmssfff")
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        ...
        Write-Error "Godot import before slice combat package 2 check failed with exit code ${LASTEXITCODE}."
        exit $LASTEXITCODE
    }
    ...
    if ($LASTEXITCODE -ne 0) {
        ...
        Write-Error "Slice combat package 2 check failed with exit code ${LASTEXITCODE}."
        exit $LASTEXITCODE
    }
    ...
    if ($unexpectedErrors.Count -gt 0) {
        ...
        Write-Error "Slice combat package 2 check reported unexpected errors."
        exit 1
    }
}
finally {
...
Write-Host "Slice combat package 2 checks passed."
