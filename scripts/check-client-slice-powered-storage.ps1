...
$checkScript = Join-Path $clientRoot "scripts/checks/slice_powered_storage_check.gd"
if (-not (Test-Path -LiteralPath $checkScript -PathType Leaf)) {
    Write-Error "Slice powered storage check script not found: ${checkScript}"
    exit 1
}

$runId = "slice-powered-storage-{0}-{1}" -f $PID, [DateTime]::UtcNow.ToString("yyyyMMddHHmmssfff")
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        ...
        Write-Error "Godot import before powered storage check failed with exit code ${LASTEXITCODE}."
        exit $LASTEXITCODE
    }
    ...
    if ($LASTEXITCODE -ne 0) {
        ...
        Write-Error "Slice powered storage check failed with exit code ${LASTEXITCODE}."
        exit $LASTEXITCODE
    }
    ...
    if ($unexpectedErrors.Count -gt 0) {
        ...
        Write-Error "Slice powered storage check reported unexpected errors."
        exit 1
    }
}
finally {
...
Write-Host "Slice powered storage checks passed."
