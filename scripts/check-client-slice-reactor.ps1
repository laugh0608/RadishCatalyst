...
$checkScript = Join-Path $clientRoot "scripts/checks/slice_reactor_check.gd"
if (-not (Test-Path -LiteralPath $checkScript -PathType Leaf)) {
    Write-Error "Slice reactor check script not found: ${checkScript}"
    exit 1
}

$runId = "slice-reactor-{0}-{1}" -f $PID, [DateTime]::UtcNow.ToString("yyyyMMddHHmmssfff")
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        ...
        Write-Error "Godot import before slice reactor check failed with exit code ${LASTEXITCODE}."
        exit $LASTEXITCODE
    }
    ...
    if ($LASTEXITCODE -ne 0) {
        ...
        Write-Error "Slice reactor check failed with exit code ${LASTEXITCODE}."
        exit $LASTEXITCODE
    }
    ...
    if ($unexpectedErrors.Count -gt 0) {
        ...
        Write-Error "Slice reactor check reported unexpected errors."
        exit 1
    }
}
finally {
...
Write-Host "Slice reactor checks passed."
