...
func _check_pollution_boundary_layer_exists_and_registers_visuals() -> void:
	...
	_expect_equal(layer.get_terrain_material_shape_count() >= 13, true, "pollution boundary registers terrain material shapes")
...
func _check_pollution_boundary_visual_priority_replaces_old_blocks() -> void:
	...
	_expect_equal(old_danger.color.a <= 0.015, true, "old pollution field no longer dominates")
	_expect_equal(old_residue.color.a <= 0.012, true, "old pollution marker no longer dominates")
