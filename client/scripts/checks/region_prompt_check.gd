...
func run() -> void:
	...
	host._expect_text_contains(ash_prompt, "盐壳余烬", "well ash crust prompt points to ash collection")
	...
	ruin_world.ensure_enemy("enemy_instance.polluted_skitter_gate_pressure", "enemy.polluted_skitter", "region.pollution_edge", 30.0)
	host._expect_text_contains(formatter.format_ruin_gate_prompt(ruin_world), "门前受扰敌人仍在压制", "ruin gate pressure prompt")
	host._expect_text_contains(formatter.format_ruin_gate_prompt(ruin_world), "带药剂回污染边界", "ruin gate pressure prompt keeps vial chain")
	ruin_world.update_enemy_health("enemy_instance.polluted_skitter_gate_pressure", 0.0, true)
	...
	host._expect_text_contains(formatter.format_deep_ruin_latch_prompt(deep_latch_world, deep_latch_character), "缺少裂相覆写栓", "deep ruin latch missing key prompt")
	...
	host._expect_text_contains(formatter.format_deep_signal_array_prompt(deep_array_world, deep_array_character), "先回基地解析裂相样块", "deep signal array blocked prompt")
	deep_array_world.quest_state.completed_quest_ids.append("quest.analyze_deep_core")
	host._expect_text_contains(formatter.format_deep_signal_array_prompt(deep_array_world, deep_array_character), "缺少裂相路由印片", "deep signal array missing imprint prompt")
	...
	host._expect_text_contains(formatter.format_phase_relay_pad_prompt(relay_pad_world), "锚定桥前线", "phase relay pad tether anchor prompt")
	...
	host._expect_text_contains(formatter.format_phase_well_lock_prompt(phase_well_lock_world, phase_well_lock_character), "先回基地完成裂相锁钥整备", "phase well lock blocked prompt")
	phase_well_lock_world.quest_state.completed_quest_ids.append("quest.refine_fault_residue")
	host._expect_text_contains(formatter.format_phase_well_lock_prompt(phase_well_lock_world, phase_well_lock_character), "缺少裂相锁钥", "phase well lock missing key prompt")
	...
	host._expect_text_contains(formatter.format_inner_phase_well_prompt(inner_phase_well_world, inner_phase_well_character), "先回基地完成回声探针整备", "inner phase well blocked prompt")
	inner_phase_well_world.quest_state.completed_quest_ids.append("quest.refine_well_flux")
	host._expect_text_contains(formatter.format_inner_phase_well_prompt(inner_phase_well_world, inner_phase_well_character), "缺少回声探针", "inner phase well missing probe prompt")
	...
	host._expect_text_contains(formatter.format_phase_well_sink_prompt(phase_well_sink_world, phase_well_sink_character), "完成盐壳整备", "phase well sink blocked prompt")
	phase_well_sink_world.quest_state.completed_quest_ids.append("quest.refine_well_ash")
	host._expect_text_contains(formatter.format_phase_well_sink_prompt(phase_well_sink_world, phase_well_sink_character), "缺少盐壳穿钉", "phase well sink missing pike prompt")
	...
	host._expect_text_contains(formatter.format_phase_well_sink_prompt(phase_well_sink_world, phase_well_sink_character), "回基地解析并继续推进碎晶沟谷断面", "phase well sink completed prompt")
	var phase_well_chamber_world := WorldState.create_default()
	var phase_well_chamber_character := CharacterState.create_default()
	host._expect_text_contains(formatter.format_phase_well_chamber_prompt(phase_well_chamber_world, phase_well_chamber_character), "完成碎晶整备", "phase well chamber blocked prompt")
	phase_well_chamber_world.quest_state.completed_quest_ids.append("quest.refine_heart_spine")
	host._expect_text_contains(formatter.format_phase_well_chamber_prompt(phase_well_chamber_world, phase_well_chamber_character), "缺少碎晶分流栓", "phase well chamber missing shunt prompt")
	...
	host._expect_text_contains(formatter.format_phase_well_chamber_prompt(phase_well_chamber_world, phase_well_chamber_character), "回基地解析并继续推进风蚀管廊断面", "phase well chamber completed prompt")
	var phase_well_loom_world := WorldState.create_default()
	var phase_well_loom_character := CharacterState.create_default()
	host._expect_text_contains(formatter.format_phase_well_loom_prompt(phase_well_loom_world, phase_well_loom_character), "完成风蚀整备", "phase well loom blocked prompt")
	phase_well_loom_world.quest_state.completed_quest_ids.append("quest.refine_weft_bundle")
	host._expect_text_contains(formatter.format_phase_well_loom_prompt(phase_well_loom_world, phase_well_loom_character), "缺少风蚀梭栓", "phase well loom missing shuttle prompt")
	...
	host._expect_text_contains(formatter.format_phase_well_loom_prompt(phase_well_loom_world, phase_well_loom_character), "回基地解析并继续推进锁相框架断面", "phase well loom completed prompt")
	var phase_well_frame_world := WorldState.create_default()
	var phase_well_frame_character := CharacterState.create_default()
	host._expect_text_contains(formatter.format_phase_well_frame_prompt(phase_well_frame_world, phase_well_frame_character), "完成锁相框架整备", "phase well frame blocked prompt")
	phase_well_frame_world.quest_state.completed_quest_ids.append("quest.refine_selvedge_strip")
	host._expect_text_contains(formatter.format_phase_well_frame_prompt(phase_well_frame_world, phase_well_frame_character), "缺少锁相键栓", "phase well frame missing key prompt")
	...
	host._expect_text_contains(formatter.format_phase_well_frame_prompt(phase_well_frame_world, phase_well_frame_character), "锚定结核已带回基地", "phase well frame completed prompt")
	host._expect_text_contains(formatter.format_phase_well_frame_prompt(phase_well_frame_world, phase_well_frame_character), "继续推进锚定桥断面", "phase well frame completed prompt keeps tether followup explicit")
	var phase_well_tether_world := WorldState.create_default()
	var phase_well_tether_character := CharacterState.create_default()
	host._expect_text_contains(formatter.format_phase_well_tether_prompt(phase_well_tether_world, phase_well_tether_character), "完成锚定桥整备", "phase well tether blocked prompt")
	phase_well_tether_world.quest_state.completed_quest_ids.append("quest.refine_tether_fiber")
	host._expect_text_contains(formatter.format_phase_well_tether_prompt(phase_well_tether_world, phase_well_tether_character), "缺少锚定桩", "phase well tether missing spike prompt")
	...
	host._expect_text_contains(formatter.format_phase_well_tether_prompt(phase_well_tether_world, phase_well_tether_character), "稳场锚核已带回基地", "phase well tether completed prompt")
	...
	host._expect_text_contains(formatter.format_phase_well_anchor_field_prompt(anchor_field_world, anchor_field_character), "缺少稳场校锚桩", "anchor field missing stake prompt")
