...
func _check_core_visual_layer_exists_and_registers_station_shapes() -> void:
	...
	_expect_equal(layer.has_station_shape("station.pollution_result_manifest"), true, "core visual layer marks pollution result manifest")
	_expect_equal(layer.has_station_shape("station.core_entry_manifest"), true, "core visual layer marks core entry manifest")
	_expect_equal(layer.has_station_shape("station.core_buffer_socket"), true, "core visual layer marks core buffer socket")
	_expect_equal(layer.has_station_shape("station.core_recovery_wave"), true, "core visual layer marks recovery wave")
	...
	_expect_equal(layer.has_station_shape("station.assetized_core_service_floor"), true, "core visual layer reuses shared service floor language")
	_expect_equal(layer.has_station_shape("station.assetized_core_pipe_bundle"), true, "core visual layer reuses pipe bundle language")
	_expect_equal(layer.has_station_shape("station.assetized_write_device_machine"), true, "core visual layer uses an assetized write device body")
	_expect_equal(layer.has_station_shape("station.shared_role_port_tokens"), true, "core visual layer registers shared role port tokens")
	_expect_equal(layer.has_station_shape("station.core_write_action_feedback"), true, "core visual layer registers core write action feedback")
	...
	_expect_equal(layer.has_flow_shape("flow.pollution_result_to_core_entry"), true, "core visual layer marks pollution result to core entry flow")
	_expect_equal(layer.has_flow_shape("flow.core_buffer_to_write_device"), true, "core visual layer marks core buffer to write device flow")
	_expect_equal(layer.has_flow_shape("flow.core_write_to_demo_hook"), true, "core visual layer marks core write to demo hook flow")
...
func _check_core_visual_operation_relation_shapes() -> void:
	...
	_expect_equal(layer.has_flow_shape("operation_relation.core.pollution_handoff_to_write"), true, "core relation links pollution handoff to write device")
...
func _check_core_visual_runtime_state_feedback() -> void:
	...
	_expect_equal(layer.get_core_handoff_shape_count() >= 6, true, "core visual layer creates pollution to core handoff shapes")
	...
	_expect_equal(layer.has_core_handoff_shape("core_handoff.pollution_result_manifest.ready"), true, "core handoff marks pollution result manifest ready")
	_expect_equal(layer.has_core_handoff_shape("core_handoff.entry_port.ready"), true, "core handoff marks entry port ready")
	_expect_equal(layer.has_core_handoff_shape("core_handoff.buffer_socket.ready"), true, "core handoff marks buffer socket ready")
	_expect_equal(layer.has_core_handoff_shape("core_handoff.write_energy.active"), true, "core handoff marks active write energy")
	...
	_expect_equal(layer.has_core_station_state_shape("core_station.feedback.stability_window.open"), true, "core visual marks opened stability window")
	_expect_equal(layer.has_core_station_state_shape("core_station.feedback.unresolved_anomaly_hook.ready"), true, "core visual marks unresolved anomaly hook after write")
	...
	_expect_equal(layer.has_core_station_state_shape("core_station.flow.core_write_to_stability_window.ready"), true, "core visual marks core write to stability window")
	_expect_equal(layer.has_core_handoff_shape("core_handoff.write_energy.completed"), true, "core handoff marks completed write energy")
	_expect_equal(layer.has_core_handoff_shape("core_handoff.recovery_wave.ready"), true, "core handoff marks recovery wave after write")
	_expect_equal(layer.has_core_handoff_shape("core_handoff.demo_hook.ready"), true, "core handoff marks demo hook after write")
	map.free()
