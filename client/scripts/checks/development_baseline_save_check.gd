...
func run() -> void:
	for definition in host.development_baseline_builder.get_baseline_definitions():
		...
		if baseline_id == "baseline.s5_phase_relay_online" or baseline_id == "baseline.s6_inner_fault_trace_ready" or baseline_id == "baseline.s7_phase_well_locator_ready" or baseline_id == "baseline.s8_phase_well_core_ready":
			host._expect_equal(
