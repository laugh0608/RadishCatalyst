...
func _check_onboarding_hints() -> void:
	...
	host._expect_text_contains(presenter.format_onboarding_hint(heart_completion_world, hint_character, ""), "碎晶心核不是收尾", "phase well heart completion onboarding keeps next package explicit")
	...
	host._expect_hint_contains(presenter, hint_world, hint_character, "quest.refine_heart_spine", "一次碎晶整备", "phase well shunt package onboarding hint")
	...
	host._expect_hint_contains(presenter, hint_world, hint_character, "quest.refine_weft_bundle", "一次风蚀整备", "weft bundle package onboarding hint")
	host._expect_hint_contains(presenter, hint_world, hint_character, "quest.inspect_phase_well_loom", "风蚀梭栓", "phase well loom onboarding hint")
	...
	host._expect_text_contains(presenter.format_direction_hint(chamber_completion_world, hint_character, ""), "回基地解析风蚀张力核", "phase well chamber completion direction points to spindle analysis")
	host._expect_text_contains(presenter.format_onboarding_hint(chamber_completion_world, hint_character, ""), "风蚀张力核不是收尾", "phase well chamber completion onboarding keeps loom package explicit")
	...
	host._expect_text_contains(presenter.format_direction_hint(loom_completion_world, hint_character, ""), "回基地解析锁相织构核", "phase well loom completion direction points to phase well frame analysis")
	host._expect_text_contains(presenter.format_onboarding_hint(loom_completion_world, hint_character, ""), "锁相织构核不是收尾", "phase well loom completion onboarding keeps frame package explicit")
	...
	host._expect_hint_contains(presenter, hint_world, hint_character, "quest.refine_selvedge_strip", "一次锁相框架整备", "phase well frame key package onboarding hint")
	host._expect_hint_contains(presenter, hint_world, hint_character, "quest.inspect_phase_well_frame", "锁相框架键栓", "phase well frame onboarding hint")
	...
	host._expect_text_contains(presenter.format_onboarding_hint(frame_completion_world, hint_character, ""), "锚定结核不是收尾", "phase well frame completion onboarding keeps tether package explicit")
	...
	host._expect_hint_contains(presenter, hint_world, hint_character, "quest.refine_tether_fiber", "一次锚定桥整备", "phase well tether spike package onboarding hint")
	host._expect_hint_contains(presenter, hint_world, hint_character, "quest.inspect_phase_well_tether", "锚定桩", "phase well tether onboarding hint")
...
func _check_status_panel_summary() -> void:
	...
	host._expect_text_contains(phase_well_sink_text, "目标：碎晶心核待解析", "status falls back to phase well heart analysis after sink")
	host._expect_text_contains(phase_well_sink_text, "回基地解析碎晶心核后", "status progress keeps phase well heart followup summary")
	...
	host._expect_text_contains(phase_well_chamber_text, "目标：风蚀张力核待解析", "status falls back to phase well spindle summary after chamber")
	host._expect_text_contains(phase_well_chamber_text, "回基地解析风蚀张力核后", "status progress keeps phase well chamber followup summary")
	...
	host._expect_text_contains(phase_well_loom_text, "目标：锁相织构核待解析", "status falls back to phase well weave core analysis after loom")
	host._expect_text_contains(phase_well_loom_text, "回基地解析锁相织构核后", "status progress keeps phase well loom followup summary")
	...
	host._expect_text_contains(phase_well_frame_text, "目标：锚定结核待解析", "status falls back to phase well knot core analysis after frame")
	host._expect_text_contains(phase_well_frame_text, "回基地解析锚定结核后", "status progress keeps phase well frame followup summary")
	...
	host._expect_text_contains(phase_well_tether_text, "目标：稳场锚核待解析", "status falls back to phase well anchor analysis summary after tether")
	...
	host._expect_text_contains(anchor_field_text, "目标：稳窗余响片已带回", "status falls back to anchor field completion summary")
	...
	host._expect_text_contains(readout_text, "目标：稳窗读数待现场校准", "status falls back to readout calibration summary")
