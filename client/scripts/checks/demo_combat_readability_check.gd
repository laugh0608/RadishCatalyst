...
func _check_formatter_exposes_player_enemy_and_supply_state() -> void:
	...
	_expect_text_contains(text, "自身：生命 58/100 警戒；防护 32/100 危险", "panel shows player vitals pressure")
	_expect_text_contains(text, "补给：1 修复凝胶x1/生命低可用", "panel shows repair gel combat value")
	_expect_text_contains(text, "2 抗污染药剂 Ix1/防护低可用", "panel shows resistance vial combat value")
	_expect_text_contains(text, "敌人：受扰掠行体 HP 30/30；中威胁；生命 / 防护承压", "panel shows enemy hp threat and pressure")
	_expect_text_contains(text, "敌人状态：扫描锁定 / 门前压力点", "panel shows enemy state")
	enemy.free()
...
func _check_attack_result_carries_combat_feedback() -> void:
	...
	_expect_text_contains(String(feedback.get("summary", "")), "命中 原生掠行体", "combat feedback names hit target")
...
func _check_hud_panel_uses_current_target_and_recent_feedback() -> void:
	...
	_expect_text_contains(label.text, "敌人：处理点掠行体 HP 35/35；中威胁；生命承压", "hud panel shows focused enemy")
	_expect_text_contains(label.text, "最近：命中 处理点掠行体：-10，敌人 HP 25", "hud panel shows recent hit")
