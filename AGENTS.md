...
- `RadishCatalyst / 异星催化` 是以异星化工生产经营为核心、探索反哺生产并逐步拓展角色成长和协作的工业科幻游戏；现行产品方向以 `docs/product/production-centered-direction.md` 为准。
- 核心设计围绕“基地服务冒险，冒险反哺基地”；化工自动化是差异化卖点，但不应成为玩家理解门槛。
- 原型方向为 Godot 4.x，脚本侧优先使用 GDScript；详细工程职责以架构文档和现有代码为准。
- 世界画面使用已确认的资产与场景组合；三维工厂使用导入模型与 `Node3D`，保留的二维切片使用 `TileMapLayer` / `Sprite2D`。程序绘制仅用于动态高亮、状态灯、选中描边和调试辅助，不作为画面主介质。
