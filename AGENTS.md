...
- 单个会话默认先执行至多 3 次图像生成调用，每次只生成 1 张图；不同素材类别仍必须在开工前拆成独立生成会话，不能借追加授权在同一会话混做多类素材。
- 图像生成会话默认只负责一个素材类别的生成与落盘，候选审阅、裁切、atlas 组装、代码 / 场景接入、Godot 验证或提交默认交给独立执行会话。生成结果已落盘并更新 manifest 后，萝卜SAMA若明确指定当前会话继续某个命名阶段，可在原会话切换到该阶段；授权只覆盖被点名阶段，不自动扩展到后续阶段、其他素材类别或无限续跑，仍须分别遵守图片读取、Godot 告知和提交边界。
...
- 这组限制是默认稳定性护栏，不由执行 agent 自行放宽；追加轮次只适用于同一素材类别的明确续作。预计需要大批量生成时，仍优先拆分会话，或由萝卜SAMA另行批准只返回文件路径的受控 API / CLI 批处理方案。

## 图像读取与审阅会话稳定性

- 单个 Codex 会话中，AI 主动读取并返回到会话上下文的图片累计默认不得超过 3 张；按图片张数计数，不按工具调用次数计数。萝卜SAMA 明确要求在当前会话继续读取或审阅时，可以突破该默认上限。
- 该默认上限统一覆盖 `view_image`、浏览器截图、Computer Use 截图、图像生成结果，以及任何一次工具调用返回的多图结果；即使已获继续授权，单次工具调用仍不得返回超过 3 张图片。
- 自动化截图脚本可以在磁盘生成多张原图，但 AI 审阅前必须先运行 `scripts/create-screenshot-contact-sheet.sh` 在磁盘合成一张带编号、等比缩略的联系表（contact sheet），优先只读取这 1 张联系表。
- 原始截图保留在磁盘，默认通过文件名、尺寸、时间、哈希和自动断言做非图像核对；确需检查像素级细节时，默认只在本会话剩余额度内追加读取至多 2 张关键原图。已获继续授权后仍只读取完成当前判断所必需的关键原图。
- 达到 3 张默认上限后，先停止加载并向萝卜SAMA 报告累计数量、继续读取的必要性与拟读取对象；未获明确要求时记录产物路径与未完成清单，交由新会话继续。萝卜SAMA 明确要求当前会话继续后，可按每次至多 3 张继续，并在每次读取后报告新的累计数量。
- 图像生成的“每轮至多 3 次调用”和本节的“默认最多 3 张图片进入上下文”独立生效；继续读取的授权不自动授权下一轮生成，继续生成的授权也不自动授权额外读取。
