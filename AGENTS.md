...
- 单个会话最多执行 3 次图像生成调用，每次只生成 1 张图；预计超过 3 张时，必须在开工前按素材类别拆成多个生成会话，不能在同一会话连续堆图。
- 图像生成会话只负责一个素材类别的生成与落盘，不同时承担候选审阅、裁切、atlas 组装、代码 / 场景接入、Godot 验证或提交；这些工作交给独立执行会话。
...
- 图像生成会话达到 3 次调用后必须停止，不因“继续”指令在原会话追加第 4 次生成；结束前在本批 `_manifest.md` 记录 thread ID、生成源路径、候选编号和未完成清单，后续生成使用新会话接续。
...
- 这组限制是会话稳定性上限，不由执行 agent 自行放宽；确需大批量生成时，应由萝卜SAMA 明确选择更多拆分会话，或另行批准使用只返回文件路径的受控 API / CLI 批处理方案。
