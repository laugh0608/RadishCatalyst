...
- 单个 Codex 会话中，AI 主动读取并返回到会话上下文的图片累计不得超过 3 张；按图片张数计数，不按工具调用次数计数。
- 该上限统一覆盖 `view_image`、浏览器截图、Computer Use 截图、图像生成结果，以及任何一次工具调用返回的多图结果；禁止单次调用返回超过 3 张图片。
...
- 原始截图保留在磁盘，默认通过文件名、尺寸、时间、哈希和自动断言做非图像核对；确需检查像素级细节时，只能在本会话剩余额度内追加读取至多 2 张关键原图。
- 达到 3 张上限后必须停止继续加载图片；仍需视觉审阅时，记录产物路径与未完成清单，交由新会话继续，不因“继续”指令在原会话突破上限。
- 图像生成会话的“最多 3 次生成调用”和本节的“最多 3 张图片进入上下文”同时生效，任一上限先达到即停止对应操作。
